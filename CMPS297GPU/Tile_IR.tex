...
\subsection{1.1. Scalable Data Parallel Computing with Tiles on GPU}
...
\subsection{1.2. Goals and Scope}
...
\subsection{1.3. Document Structure}

\item Introduction this section.
\item Programming Model describes the programming model of \textbf{Tile IR} .
\item Syntax defines the syntax of \textbf{Tile IR} programs used throughout the specification.
\item Binary Format describes the \textbf{Tile IR} byte code and its binary format.
\item Type System defines the type system of \textbf{Tile IR} .
\item Semantics describes the semi-formal semantics of \textbf{Tile IR} .
\item Memory Model describes the \textbf{Tile IR} memory model.
\item Operations a listing of the full set of \textbf{Tile IR} operations.
\item Debug Info describes the debug information format for \textbf{Tile IR} programs.
\item Stability describes the stability guarantees of the \textbf{Tile IR} .
\item Appendix contains relevant reference materials, including full programs and references.
\end{enumerate}
...
\subsubsection{2.1.1. What’s different about tile programming?}
...
\subsubsection{2.1.2. Kernel I/O}
...
All values in \textbf{Tile IR} are either tensors, or tensor views (see Tensor View ). A tensor is an n-dimensional rectangular array, described by its rank (number of dimensions), the shape (extent along each dimension), and its primitive element type.
...
We use cuda\_tile.iota which constructs a range tensor that counts from \texttt{0} to \texttt{n - 1} forming an \texttt{n} sized vector.
...
Each tile kernel can be launched with a 1-d, 2-d, or 3-d grid. Each tile block can query its position in the grid using cuda\_tile.get\_tile\_block\_id and query the first, second, or third dimension index by varying the argument.

Tile kernels can also observe the grid dimensions in a similar way using cuda\_tile.get\_num\_tile\_blocks .
...
\subsubsection{2.2.1. Implementing GEMM}
...
\subsubsection{2.2.2. GEMM with a Single Block}
...
We then load the input arguments, compute the MMA operation (see cuda\_tile.mmaf for floating-point and cuda\_tile.mmai for integers) to compute the output tile, and then store it.
...
\subsubsection{2.2.3. GEMM Block by Block}
...
We also create a shared range from \texttt{(0, 63)} using cuda\_tile.iota again for the reduction dimension.
...
\subsubsection{2.2.4. Looping Over Tiles}
...
\subsection{2.3. Structured Pointers}
...
\includesvg[width=0.8\textwidth]{C:/Users/hy-wu.DESKTOP-G355NC5/1/note/tile/files/global_memory.svg}
...
In order to both simplify the programming model of \textbf{Tile IR} and to improve the efficiency of user programs \textbf{Tile IR} also has a structured pointer type known as a tensor view . A tensor view can be constructed from raw pointers via cuda\_tile.make\_tensor\_view and attaches shape and stride information to the pointer and effectively represents a typed pointer to a tensor.


When constructing a tensor view , we convert the raw pointer into a tensor using static or dynamic shape and stride information, which is done via cuda\_tile.make\_tensor\_view . This op attaches shape and stride information to the pointer and effectively converts a typed pointer to a tensor.


\begin{figure}[h]\centering
\includesvg[width=0.8\textwidth]{C:/Users/hy-wu.DESKTOP-G355NC5/1/note/tile/files/tensor_view.svg}
\caption{A view of global memory}
\end{figure}
The base pointer to A points to an allocation that lives in global memory. \#
...
Once you have constructed a tensor view, we can make use of cuda\_tile.make\_partition\_view to perform tiling of the underlying tensor.


\begin{figure}[h]\centering
\includesvg[width=0.8\textwidth]{C:/Users/hy-wu.DESKTOP-G355NC5/1/note/tile/files/tiled.svg}
\caption{A view of global memory}
\end{figure}
The base pointer to A points to an allocation that lives in global memory. \#
...
\subsubsection{2.4.1. Vector Addition with Views}

We can now implement a more complex vector operation with SAXPY or “Single-Precision A·X Plus Y” (a common

BLAS operation). We can use \texttt{tensor view} and cuda\_tile.make\_partition\_view to implement this operation for arbitrary sized vectors.
...
We use cuda\_tile.make\_partition\_view to create views for \texttt{\%x} and \texttt{\%y} which represent a \texttt{(M/128 x N/256)} tensor of tiles. Each tile will have the size specified by tile parameter of the partition type.
...
cuda\_tile.load\_view\_tko allows us to load the tile specified by \texttt{\%view[\%x, \%y]} for both \%X and \%Y .
...
\subsubsection{2.4.2. Dynamic GEMM with Views}
...
The \textbf{Tile IR} compiler can optimize the memory loads and stores of Tensor View if the alignment of the underlying pointers and strides are known. If these are statically known then we can infer the alignment directly but if they are dynamic, as they are in this case, we can use the cuda\_tile.assume operation to inform the compiler that the pointer is properly aligned.
...
We then read the the tile block grid coordinates using cuda\_tile.get\_tile\_block\_id .
...
Due to the \texttt{K} dimension being dynamic we could do calculations ourselves to compute the size of the index space of the reduction but \textbf{Tile IR} provides an operation to do this for us. cuda\_tile.get\_index\_space\_shape to get the size of the index space, and thus the size of the reduction loop.
...
No we can use a cuda\_tile.for loop to iterate over the tile blocks.
...
The iter\_values are loop carried variables of the body which are initialized in the loop header and are updated each iteration by yielding them with the cuda\_tile.continue operation.
...
\subsection{2.5. Cross TileBlock Communication}
...
\subsubsection{4.1.1. Fixed-Width Integers}
...
\subsubsection{4.1.2. Variable-Width Integers}
...
\subsubsection{4.2.1. Magic Number}
...
\subsection{4.3. Operation Opcodes and Encodings}
...
\subsection{4.4. Operation Encoding Details}
...
\subsection{4.5. Section Ordering}
...
\subsubsection{5.1.1. Fundamental Types}
...
\subsubsection{5.1.2. Alternative Types}
...
(i.e., nested pointer types are not supported). For details about converting between different pointer types, or integers see cuda\_tile.bitcast .



\subsection{5.3. Tensor Types}
...
We use \texttt{tile<MxNxKxE>} where \texttt{M} , \texttt{N} , \texttt{K} are integer numbers and \texttt{E} is an element type as syntax for tile types, see Syntax for more details.
...
\subsubsection{5.3.2. Tensor View}
...
elem[i0,...,in]=baseptr+∑m=0nim∗sm where the s m are the striding factors of the tensor view and b a s e p t r is the start address in global memory.


Other than the tile type, a tensor view supports dynamic extents in the shape vector. We use the \texttt{?} character to denote these. Dynamic extents are bound at runtime to values when the view type is constructed using a cuda\_tile.make\_tensor\_view operation.
...
A tensor view is constructed using the cuda\_tile.make\_tensor\_view operation and consists of the components:
...
\subsubsection{5.3.3. Subview Types}
...
Subviews describe a mapping from an index space to a space of statically-sized tiles loaded from a tensor view. They define the necessary index computations performed by a cuda\_tile.load\_view\_tko and cuda\_tile.store\_view\_tko when accessing elements from a tensor view. A subview’s size is defined by the cardinality of its index space.
...
The partition view structure is created using the cuda\_tile.make\_partition\_view constructor.
...
location[i0,…,in]=baseptr+∑m=0nIm⋅ceildiv(Sm,Tm)⋅stm
...
\subsection{5.4. Type Equivalence}
...
It does \textbf{not} attempt to formalize every possible behavior that might be admitted by an axiomatic formulation or a small step operational semantics. For understanding an even more informal presentation of the language and its core concepts see the Programming Model section.
...
\subsection{6.1. The Abstract Machine}

The \textbf{Tile IR} abstract machine state S is a tuple consisting of the following components, each explained below:


\begin{itemize}
\item A well-formed module Mod which stores one of more items, discussed below in Modules .
...
\begin{itemize}
\item a global variable definition
\item a tile kernel
\item a tile function
...
A global variable can be modified by using the cuda\_tile.get\_global operation to obtain a pointer which can be used to read and write to the global variable.
...
\subsubsection{6.2.3. Tile Function}
...
\subsubsection{6.2.4. Function Bodies}
...
\item The program type checks according to the rules specified in Type System and the operator type signatures are specified in Operations .
...
\subsection{6.3. Values}

\textbf{Tile IR} has a small set of types as described in Type System but we only have two types of values.


\begin{itemize}
\item Pointers , which represent a memory address.
\item Tiles , or an N-dimensional array of scalars.
\item Views , which represent a structured view of memory.

\subsubsection{6.3.1. Pointers}
...
For NumPy low-precision types we provide the equivalent in terms of the ml\_dtypes library a standard collection of low-precision NumPy data types.
...
\textbf{Tile IR} provides a set of view types as described in Tensor View .


Views provide a structured views of memory by enriching a pointer with additional data.

Views have their own set of memory operations cuda\_tile.load\_view\_tko and cuda\_tile.store\_view\_tko which vary in behavior based on the view type and make use of the additional metadata. Due to the face that views are not a singular type, but a family of types each concrete view type has its own value representation.


Our first-order view type is Tensor View . A tensor view value is logically a tuple of ( p t r , s h a p e → , s t r i d e s → , d i m g r o u p s → ) , and load and store operate on the entire tensor view. The index space of this view is rank-0 (i.e., there is only a single element in the index space).
...
For example a tensor view with shape ( 1024 , 1024 ) may be partitioned into a grid of 64 × 32 tiles, resulting in a partition view with shape ( 16 , 32 ) . See Type System for more details on the different types of views.
...
tile\textbackslash{}\_grid(i→)=\{i0where grid=(x)i0⋅x+i1where grid=(x,y)i0⋅x+i1⋅y+i2where grid=(x,y,z)where i→=(i0,…,in)and 0≤ik<gridk for all k≤3
...
Global memory \textbf{is} the same global device memory that is used by CUDA programs and described in the PTX Specification .


The specification elaborates the intricacies of the \textbf{Tile IR} memory model in Memory Model .



\subsection{6.7. Tile Block}
...
\subsection{6.8. Execution Semantics}

\textbf{Tile IR} program execution starts with a kernel launch. The kernel launch API is uniform for all CUDA kernels and is specified in the CUDA Runtime API .


\subsubsection{6.8.1. Initialization}
...
\subsubsection{6.8.2. Forward Progress}
...
\subsubsection{6.8.3. Tile Block Execution}
...
\subsubsection{6.8.4. Termination}

A tile block will terminate when the tile block’s function body reaches the final statement. Tile kernels must terminate with a return operation cuda\_tile.return which signals the end of the execution.
...
\subsection{7.1. Memory model operations}
...
\subsection{7.3. Memory ordering}
...
\subsection{7.5. Tokens and token order}
...
\subsection{7.11. PTX interoperability}
...
\subsection{7.12. Hazards and intuition in the \textbf{Tile IR} Memory Model}

\subsubsection{7.12.1. Token ordered operations reading from the future}

Store buffering and load buffering behavior are visible when using token ordered operations without token constraints.



\subsubsection{7.12.2. Race hazards within a single tile block thread}
...
\subsubsection{7.12.3. Some intuition on how to use token ordering}
...
\subsection{8.2. Operation Design Considerations}
...
\subsubsection{8.2.1. Explicit Broadcast}
...
For example to use the cuda\_tile.offset operation to add an offset tile to a pointer, the pointer and offset must be reshaped or broadcasted to have the same shape using the cuda\_tile.reshape or cuda\_tile.broadcast operations.



\subsubsection{8.2.2. Distinct Floating-Point and Integer Operations}
...
For example, the cuda\_tile.addf operation supports a rounding attribute, but the addi operation does not.



\subsubsection{8.2.3. Explicit Overflow Annotations}

Some operations such as cuda\_tile.addi support an explicit overflow annotation that expresses the expected overflow behavior of the operation.
...
\begin{itemize}
\item \textbf{source} ( tile ) - The tile to broadcast. 13.1


\paragraph{Results}

\item \textbf{result} ( tile ) - The broadcasted tile. 13.1
...
\begin{itemize}
\item \textbf{lhs} ( tile ) - The left hand side operand. 13.1
\item \textbf{rhs} ( tile ) - The right hand side operand. 13.1
\item \textbf{dim} ( i64 ) - The dimension along which to concatenate. 13.1


\paragraph{Results}

\item \textbf{result} ( tile ) - The concatenated result tile. 13.1
...
\item \texttt{lhs} , \texttt{rhs} and \texttt{result} must have the same element type ( tile ).
...
\end{itemize}
See cuda\_tile.cat\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{comparison\_predicate} ( ComparisonPredicate ) - The comparison predicate. 13.1
\item \textbf{comparison\_ordering} ( ComparisonOrdering ) - The comparison ordering. 13.1
\item \textbf{lhs} ( tile < f16 | bf16 | f32 | f64 > ) - The left hand side operand. 13.1
\item \textbf{rhs} ( tile < f16 | bf16 | f32 | f64 > ) - The right hand side operand. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < i1 > ) - The result of the comparison. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{lhs} and \texttt{rhs} must have the same shape and element type ( tile < f16 | bf16 | f32 | f64 > ).
...
\end{itemize}
See cuda\_tile.cmpf\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{comparison\_predicate} ( ComparisonPredicate ) - The comparison predicate. 13.1
\item \textbf{lhs} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The left hand side operand. 13.1
\item \textbf{rhs} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The right hand side operand. 13.1
\item \textbf{signedness} ( Signedness ) - Interpret integer(s) as \texttt{signed} or \texttt{unsigned} 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{lhs} and \texttt{rhs} must have the same shape and element type ( tile < i1 | i8 | i16 | i32 | i64 > ).
\item Result type has i1 element type and same shape as operands
\item The operation’s result type may be inferred from its operands and attributes.
...
\end{itemize}
See cuda\_tile.cmpi\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{value} ( DenseConstant ) - The constant value to create. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 | f16 | bf16 | f32 | f64 | fp8e4m3fn | fp8e5m2 | tf32 > ) - The constant tile. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{value} and \texttt{result} must have the same shape and element type ( DenseConstant ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\end{itemize}
See cuda\_tile.constant\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{sym\_name} ( Symbol ) - The name of the function. 13.1
\item \textbf{function\_type} ( Type ) - The type of the function. 13.1
\item \textbf{arg\_attrs} ( Attributes ) - The argument attributes of the function: none of these are supported by TileIR at the moment. 13.1
\item \textbf{res\_attrs} ( Attributes ) - The result attributes of the function: none of these are supported by TileIR at the moment. 13.1
\item \textbf{optimization\_hints} ( OptimizationHints ) - Compiler architecture-specific optimization hints 13.1
...
\begin{itemize}
\item \texttt{num\_cta\_in\_cga} - suggest the number of CTAs in a CGA (which must be the power of 2 less than or equal to 16) for cuda\_tile.entry .
\item \texttt{allow\_tma} - suggest whether to use TMA for cuda\_tile.load\_view\_tko and cuda\_tile.store\_view\_tko .
\item \texttt{latency} - latency hint for cuda\_tile.load\_view\_tko and cuda\_tile.store\_view\_tko .
...
\begin{itemize}
\item \textbf{source} ( tile ) - The source tile to extract from. 13.1
\item \textbf{indices} ( Variadic < tile < i32 > > ) - The indices of the slice to extract. 13.1


\paragraph{Results}

\item \textbf{result} ( tile ) - The extracted subtile. 13.1
...
\end{itemize}
See cuda\_tile.extract\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{name} ( Symbol ) - The name of the global variable. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < ptr > ) - The result of the get\_global operation. 13.1


\paragraph{Description}

\end{itemize}
The \texttt{get\_global} operation returns a pointer to the specified \texttt{global} variable. A global variable is a form of static global memory allocation that can be declared using the cuda\_tile.global operation.
...
\end{itemize}
See cuda\_tile.get\_global\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{gridSize\_x} ( tile < i32 > ) - The number of tile blocks in dimension \texttt{x} . 13.1
\item \textbf{gridSize\_y} ( tile < i32 > ) - The number of tile blocks in dimension \texttt{y} . 13.1
\item \textbf{gridSize\_z} ( tile < i32 > ) - The number of tile blocks in dimension \texttt{z} . 13.1
...
\end{itemize}
See cuda\_tile.get\_num\_tile\_blocks\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{blockId\_x} ( tile < i32 > ) - The tile block ID for dimension \texttt{x} . 13.1
\item \textbf{blockId\_y} ( tile < i32 > ) - The tile block ID for dimension \texttt{y} . 13.1
\item \textbf{blockId\_z} ( tile < i32 > ) - The tile block ID for dimension \texttt{z} . 13.1
...
\begin{itemize}
\item \textbf{sym\_name} ( Symbol ) - The name of the global variable. 13.1
\item \textbf{value} ( DenseConstant ) - The value to initialize the allocation with. 13.1
\item \textbf{alignment} ( i64 ) - The alignment of the buffer. 13.1
...
The \texttt{global} operation statically allocates a mutable 1-dimensional location in global memory and initializes it using \texttt{value} . The initialization of the allocation is performed at CUDA module load time. The lifetime of the allocation is the same as the lifetime of the module.


The allocation may be read or written to by first using cuda\_tile.get\_global to obtain a pointer to the the memory and then read using cuda\_tile.load\_ptr\_tko or written to using cuda\_tile.store\_ptr\_tko .
...
\end{itemize}
See cuda\_tile.global\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The result of the iota operation. 13.1
...
\begin{itemize}
\item \textbf{lhs} ( tile < f16 | bf16 | f32 | f64 | tile < tf32 > > ) - The left hand side matrix operand. 13.1
\item \textbf{rhs} ( tile < f16 | bf16 | f32 | f64 | tile < tf32 > > ) - The right hand side matrix operand. 13.1
\item \textbf{acc} ( tile < f16 | f32 | f64 > ) - The accumulator matrix operand. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < f16 | f32 | f64 > ) - The result matrix after multiplication and accumulation. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{acc} and \texttt{result} must have the same shape and element type ( tile < f16 | f32 | f64 > ).
\item \texttt{lhs} and \texttt{rhs} must have the same element type ( tile < f16 | bf16 | f32 | f64 | tile < tf32 > > ).
...
\end{itemize}
See cuda\_tile.mmaf\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{lhs} ( tile < i8 > ) - The left hand side matrix operand. 13.1
\item \textbf{rhs} ( tile < i8 > ) - The right hand side matrix operand. 13.1
\item \textbf{acc} ( tile < i32 > ) - The accumulator matrix operand. 13.1
\item \textbf{signedness\_lhs} ( Signedness ) - The signedness of the \texttt{lhs} operand. 13.1
\item \textbf{signedness\_rhs} ( Signedness ) - The signedness of the \texttt{rhs} operand. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < i32 > ) - The result matrix after multiplication and accumulation. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{acc} and \texttt{result} must have the same shape and element type ( tile < i32 > ).
\item \texttt{lhs} and \texttt{rhs} must have the same element type ( tile < i8 > ).
\item \texttt{lhs} , \texttt{rhs} and \texttt{acc} must have the same rank.
\item The operation’s result type may be inferred from its operands and attributes.
...
\end{itemize}
See cuda\_tile.mmai\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{sym\_name} ( Symbol ) - The name of the module. 13.1
...
\begin{itemize}
\item \textbf{ptr} ( ptr ) - The base pointer tile to advance. 13.1
\item \textbf{offset} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The offset tile to add to the pointer. 13.1


\paragraph{Results}

\item \textbf{result} ( ptr ) - The resulting pointer tile after advancement. 13.1
...
\item \texttt{result} and \texttt{ptr} must have the same shape and element type ( ptr ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\begin{itemize}
\item \textbf{source} ( tile < i1 | i8 | i16 | i32 | i64 | f16 | bf16 | f32 | f64 | fp8e4m3fn | fp8e5m2 | tf32 > ) - The input tile. 13.2


\paragraph{Results}

\item \textbf{result} ( tile < i8 > ) - The packed tile. 13.2
...
\end{itemize}
See cuda\_tile.pack\_0 for the full example listing.
...
See cuda\_tile.pack\_1 for the full example listing.
...
\begin{itemize}
\item \textbf{source} ( tile ) - The input tile. 13.1
\item \textbf{permutation} ( Array < i32 > ) - The permutation of the dimensions. 13.1


\paragraph{Results}

\item \textbf{result} ( tile ) - The permuted tile. 13.1
...
\end{itemize}
See cuda\_tile.permute\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{operands} ( Variadic < tile > ) - The set of tiles to reduce. 13.1
\item \textbf{dim} ( i32 ) - The index of the dimension to perform reduction on. 13.1
\item \textbf{identities} ( Array ) - The reduction identities for each operand. 13.1


\paragraph{Results}

\item \textbf{results} ( Variadic < tile > ) - The set of reduced tiles. 13.1
...
\end{itemize}
See cuda\_tile.reduce\_0 for the full example listing.
...
See cuda\_tile.reduce\_1 for the full example listing.
...
\begin{itemize}
\item \textbf{source} ( tile ) - The source tile to reshape. 13.1


\paragraph{Results}

\item \textbf{result} ( tile ) - The reshaped tile. 13.1
...
\end{itemize}
See cuda\_tile.reshape\_0 for the full example listing.
...
See cuda\_tile.reshape\_1 for the full example listing.
...
\begin{itemize}
\item \textbf{operands} ( Variadic < tile > ) - The a set of tiles to scan. 13.1
\item \textbf{dim} ( i32 ) - The index of the dimension along which to scan. 13.1
\item \textbf{reverse} ( bool ) - Whether to scan in reverse order. 13.1
\item \textbf{identities} ( Array ) - The identities of the scan operation. 13.1


\paragraph{Results}

\item \textbf{results} ( Variadic < tile > ) - The resulting tiles from the scan operation. 13.1
...
\end{itemize}
See cuda\_tile.scan\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{cond} ( tile < i1 > ) - The condition tile. 13.1
\item \textbf{val\_if\_true} ( tile ) - The value if true tile. 13.1
\item \textbf{val\_if\_false} ( tile ) - The value if false tile. 13.1


\paragraph{Results}

\item \textbf{result} ( tile ) - The tile of selected values. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{val\_if\_true} , \texttt{val\_if\_false} and \texttt{result} must have the same shape and element type ( tile ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\begin{itemize}
\item \textbf{source} ( tile < i8 > ) - The input i8 tile. 13.2


\paragraph{Results}

\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 | f16 | bf16 | f32 | f64 | fp8e4m3fn | fp8e5m2 | tf32 > ) - The output unpacked tile. 13.2
...
\end{itemize}
See cuda\_tile.unpack\_0 for the full example listing.
...
See cuda\_tile.unpack\_1 for the full example listing.




\subsection{8.4. Conversions}
...
cuda\_tile.bitcast preserves the contents of the input but allows for changing of element types, cuda\_tile.exti and cuda\_tile.trunci change the width of integer tiles, cuda\_tile.ftoi and cuda\_tile.itof convert floating-point tiles to integer tiles and vice versa, and cuda\_tile.ftof converts between different floating-point types.
...
\begin{itemize}
\item \textbf{source} ( tile < i1 | i8 | i16 | i32 | i64 | f16 | bf16 | f32 | f64 | fp8e4m3fn | fp8e5m2 | tf32 > ) - The source tile to cast. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 | f16 | bf16 | f32 | f64 | fp8e4m3fn | fp8e5m2 | tf32 > ) - The casted tile. 13.1
...
Pointer types must use cuda\_tile.ptr\_to\_int or cuda\_tile.int\_to\_ptr instead.
...
\begin{itemize}
\item \textbf{from} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The input integer tile to extend. 13.1
\item \textbf{signedness} ( Signedness ) - Interpret integer(s) as \texttt{signed} or \texttt{unsigned} 13.1


\paragraph{Results}

\item \textbf{to} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The extended integer tile. 13.1
...
\begin{itemize}
\item \textbf{from} ( tile < f16 | bf16 | f32 | f64 | fp8e4m3fn | fp8e5m2 | tf32 > ) - The input floating-point tile. 13.1
\item \textbf{rounding\_mode} ( RoundingMode ) - The rounding mode for the operation. 13.1


\paragraph{Results}

\item \textbf{to} ( tile < f16 | bf16 | f32 | f64 | fp8e4m3fn | fp8e5m2 | tf32 > ) - The result floating-point tile. 13.1
...
\item \textbf{to} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The result integer tile. 13.1
...
In contrast to a cuda\_tile.bitcast which is bits preserving, this preserves the numerical value of the tile, rounded towards zero to the nearest integer of the provided type.
...
\begin{itemize}
\item \textbf{from} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The input integer tile. 13.1
\item \textbf{signedness} ( Signedness ) - Interpret integer(s) as \texttt{signed} or \texttt{unsigned} 13.1
\item \textbf{rounding\_mode} ( RoundingMode ) - The rounding mode for the operation. 13.1
...
\item \textbf{to} ( tile < f16 | bf16 | f32 | f64 | fp8e4m3fn | fp8e5m2 | tf32 > ) - The converted floating-point tile. 13.1
...
In contrast to cuda\_tile.bitcast , this preserves the numerical value of the tile, rounded to the nearest floating-point number of the provided type.
...
\begin{itemize}
\item \textbf{source} ( tile < i64 > ) - The input tile of integers. 13.1


\paragraph{Results}

\item \textbf{result} ( ptr ) - The output tile of pointers. 13.1
...
\begin{itemize}
\item \textbf{source} ( ptr ) - The input tile of pointers. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < i64 > ) - The output tile of integers. 13.1
...
The inverse of this operation is cuda\_tile.int\_to\_ptr .
...
\begin{itemize}
\item \textbf{source} ( ptr ) - Tile with source pointer element type. 13.1


\paragraph{Results}

\item \textbf{result} ( ptr ) - Tile with target pointer element type. 13.1
...
In order to perform those conversions, use cuda\_tile.ptr\_to\_int or cuda\_tile.int\_to\_ptr .
...
\begin{itemize}
\item \textbf{from} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The input integer tile to truncate. 13.1
\item \textbf{overflow} ( IntegerOverflow ) - The overflow behavior of the operation. 13.1


\paragraph{Results}

\item \textbf{to} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The truncated integer tile. 13.1
...
A notable difference is that we allow the nesting of control flow operations for example a cuda\_tile.if may appear inside a cuda\_tile.loop or cuda\_tile.for .
...
\begin{itemize}
\item cuda\_tile.if which implements conditional branching.
\item cuda\_tile.loop which implements a loop with arbitrary exit conditions.
\item cuda\_tile.for which implements a range-based loop with a fixed number of iterations.
...
\begin{itemize}
\item \textbf{condition} ( tile < i1 > ) - The condition tile to check. 13.1
\item \textbf{message} ( String ) - The error message to display if assertion fails. 13.1
...
See cuda\_tile.assert\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{operands} ( Variadic < Any > ) - The operands to yield to the parent loop upon termination. 13.1
...
The \texttt{break} operation is a terminator operation of a cuda\_tile.loop .
...
\end{itemize}
See cuda\_tile.break\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{operands} ( Variadic < Any > ) - The values to yield to the parent loop. 13.1
...
The \texttt{continue} operation represents a block terminator that returns control to a loop operation, such as cuda\_tile.for and cuda\_tile.loop . The operation may yield any number of \texttt{\$operands} to the parent loop upon termination.
...
\end{itemize}
See cuda\_tile.continue\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{lowerBound} ( tile < any > ) - The lower bound of the loop. 13.1
\item \textbf{upperBound} ( tile < any > ) - The upper bound of the loop. 13.1
\item \textbf{step} ( tile < any > ) - The step of the loop. 13.1
\item \textbf{initValues} ( Variadic < Any > ) - The initial values of the loop-carried values. 13.1
\item \textbf{unsignedCmp} ( Flag ) - If present, use unsigned integer comparison for loop termination. 13.2


\paragraph{Results}

\item \textbf{resultValues} ( Variadic < Any > ) - The values of the loop-carried variables after loop termination. 13.1
...
The body of the loop must be terminated by a cuda\_tile.continue that yields the next iteration’s value for each loop carried variable.
...
\begin{itemize}
\item Loop carried variables can not be a tensor\_view or view type.
\item \texttt{for} operations cannot terminate early and must end in a cuda\_tile.continue .


\paragraph{Constraints}

\item The operation must define scope when stack allocations are freed automatically.
\item \texttt{lowerBound} , \texttt{upperBound} and \texttt{step} must have the same shape and element type ( tile < any > ).
\item \texttt{initValues} and \texttt{resultValues} must have the same shape and element type ( Variadic < Any > ).
\item The operation must provide custom parsing and printing methods.
\item The operation only has an effect if and only if it the region’s operation have an effect.
\item Each provided region must contain exactly one block.
...
\end{itemize}
See cuda\_tile.for\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{condition} ( tile < i1 > ) - The condition of the if operation. 13.1


\paragraph{Results}

\item \textbf{results} ( Variadic < Any > ) - The results of the if operation. 13.1
...
The \texttt{if} operation may produce results by yielding values in each branch using cuda\_tile.yield .
...
\begin{itemize}
\item Results of \texttt{if} must not be a tensor\_view or view type.
...
\end{itemize}
See cuda\_tile.if\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{initValues} ( Variadic < Any > ) - The initial values of the loop. 13.1


\paragraph{Results}

\item \textbf{resultValues} ( Variadic < Any > ) - The result values of the loop. 13.1


\paragraph{Description}

\end{itemize}
The \texttt{loop} operation represents an, unstructured, infinite loop that executes until a cuda\_tile.break is reached.
...
The loop will execute the body of the loop until a cuda\_tile.break is dynamically executed.
...
\begin{itemize}
\item a cuda\_tile.continue that yields the next iteration’s value for each loop carried variable.
\item a cuda\_tile.break that terminates the loop and yields the final loop carried values.
...
\end{itemize}
See cuda\_tile.loop\_0 for the full example listing.
...
See cuda\_tile.loop\_1 for the full example listing.
...
See cuda\_tile.loop\_2 for the full example listing.
...
See cuda\_tile.loop\_3 for the full example listing.
...
\begin{itemize}
\item \textbf{operands} ( Variadic < Any > ) - The values to return. 13.1
...
\begin{itemize}
\item cuda\_tile.entry operations do not produce return value(s) and thus \texttt{return} may be used to terminate the execution of the kernel by invoking the operation with no operands
...
\end{itemize}
See cuda\_tile.return\_0 for the full example listing.
...
See cuda\_tile.return\_1 for the full example listing.
...
\begin{itemize}
\item \textbf{operands} ( Variadic < Any > ) - The operands to yield to the parent operation. 13.1
...
Unlike standard MLIR control flow dialects \texttt{yield} is not used for loop control flow, see cuda\_tile.break and cuda\_tile.continue for loop control flow.
...
\end{itemize}
See cuda\_tile.yield\_0 for the full example listing.




\subsection{8.6. Memory}
...
\begin{itemize}
\item Tile of pointer based memory operations such as cuda\_tile.load\_ptr\_tko and cuda\_tile.store\_ptr\_tko which load and store tiles from and to global memory.
\item View based memory operations such as cuda\_tile.load\_view\_tko and cuda\_tile.store\_view\_tko which load and store tiles from and to views.
\item Atomic memory operations such as cuda\_tile.atomic\_rmw\_tko and cuda\_tile.atomic\_cas\_tko which perform atomic operations on global memory.
...
\begin{itemize}
\item \textbf{tokens} ( Variadic < token > ) - The input tokens to join. 13.1


\paragraph{Results}

\item \textbf{result} ( token ) - The joined token. 13.1
...
\begin{itemize}
\item \textbf{memory\_ordering\_semantics} ( MemoryOrderingSemantics ) - The memory ordering semantics for the load operation. 13.1
\item \textbf{memory\_scope} ( MemoryScope ) - The memory scope for the atomic operation. 13.1
\item \textbf{source} ( ptr ) - The source tile of pointers. 13.1
\item \textbf{mask} ( tile < i1 > ) - The mask for the load operation. 13.1
\item \textbf{paddingValue} ( tile < i1 | i8 | i16 | i32 | i64 | f16 | bf16 | f32 | f64 | fp8e4m3fn | fp8e5m2 | tf32 > ) - The padding value for the load operation. 13.1
\item \textbf{token} ( token ) - The token for the load operation. 13.1
\item \textbf{optimization\_hints} ( OptimizationHints ) - Optimization hints for operation 13.1


\paragraph{Results}

\item \textbf{result} ( tile ) - The result of the load operation. 13.1
\item \textbf{result\_token} ( token ) - The result token of the load operation. 13.1
...
\end{itemize}
See cuda\_tile.load\_ptr\_tko\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{result} ( token ) - A fresh token with no prior dependencies. 13.1
...
\begin{itemize}
\item \textbf{memory\_ordering\_semantics} ( MemoryOrderingSemantics ) - The memory ordering semantics. 13.1
\item \textbf{memory\_scope} ( MemoryScope ) - The optional memory scope. 13.1
\item \textbf{destination} ( ptr ) - The destination pointer tile. 13.1
\item \textbf{value} ( tile ) - The value tile to store. 13.1
\item \textbf{mask} ( tile < i1 > ) - The optional mask for selective storage. 13.1
\item \textbf{token} ( token ) - The optional token for operation ordering. 13.1
\item \textbf{optimization\_hints} ( OptimizationHints ) - Optimization hints for operation 13.1


\paragraph{Results}

\item \textbf{result\_token} ( token ) - The result token for synchronization. 13.1
...
\subsection{8.7. Floating Point}

\end{itemize}
\textbf{Tile IR} contains a set of typed arithmetic operations which implement familiar arithmetic operations on floating-point types for integer operations see Integer .
...
\subsubsection{8.7.2. Floating-Point Math}
...
\begin{itemize}
\item \textbf{source} ( tile < f16 | bf16 | f32 | f64 > ) - The input float tile. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The absolute value of the input tile. 13.1
...
Element-wise floating-point arithmetic operations are performed by the target architecture’s native floating-point instructions. If the \texttt{rounding} modifier is specified, the particular rounding mode will be applied to each element of the result. See Floating Point for more details.
...
\item \texttt{source} and \texttt{result} must have the same shape and element type ( tile < f16 | bf16 | f32 | f64 > ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\item \textbf{rhs} ( tile < f16 | bf16 | f32 | f64 > ) - The right hand side operand. 13.1
\item \textbf{rounding\_mode} ( RoundingMode ) - The rounding mode for the operation. 13.1
\item \textbf{flush\_to\_zero} ( Flag ) - If set, flushes subnormal inputs and results to sign-preserving zero. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The sum of the input tiles. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{lhs} , \texttt{rhs} and \texttt{result} must have the same shape and element type ( tile < f16 | bf16 | f32 | f64 > ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\begin{itemize}
\item \textbf{x} ( tile < f16 | bf16 | f32 | f64 > ) - The input x float tile. 13.2
\item \textbf{y} ( tile < f16 | bf16 | f32 | f64 > ) - The input y float tile. 13.2


\paragraph{Results}

\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The element-wise result tile. 13.2
...
This operation is emulated in \texttt{:code:}f32`\texttt{ when executed on half-precision inputs ( }:code:\texttt{f16}\texttt{ and }:code:\texttt{bf16}` ). See Floating Point for more details.
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{x} , \texttt{y} and \texttt{result} must have the same shape and element type ( tile < f16 | bf16 | f32 | f64 > ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\end{itemize}
See cuda\_tile.atan2\_0 for the full example listing.
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The ceiling of the input tile. 13.1
...
\end{itemize}
See cuda\_tile.ceil\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{source} ( tile < f16 | bf16 | f32 | f64 > ) - The input floating-point tile. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The hyperbolic cosine of the input tile. 13.1
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The cosine of the input tile. 13.1
...
\end{itemize}
See cuda\_tile.cos\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{lhs} ( tile < f16 | bf16 | f32 | f64 > ) - The dividend input floating-point tile. 13.1
\item \textbf{rhs} ( tile < f16 | bf16 | f32 | f64 > ) - The divisor input floating-point tile. 13.1
\item \textbf{rounding\_mode} ( RoundingMode ) - The rounding mode for the operation. 13.1
\item \textbf{flush\_to\_zero} ( Flag ) - If set, flushes subnormal inputs and results to sign-preserving zero. 13.1
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The result of the \texttt{divf} operation. 13.1
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The result of raising 2 to the power of the input tile. 13.1
...
\end{itemize}
See cuda\_tile.exp2\_0 for the full example listing.
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The exponential of the input tile. 13.1
...
\end{itemize}
See cuda\_tile.exp\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{source} ( tile < f16 | bf16 | f32 | f64 > ) - The input tile to the floor operation. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The result of the floor operation. 13.1
...
\end{itemize}
See cuda\_tile.floor\_0 for the full example listing.
...
\item \textbf{lhs} ( tile < f16 | bf16 | f32 | f64 > ) - The left hand side operand. 13.1
\item \textbf{rhs} ( tile < f16 | bf16 | f32 | f64 > ) - The right hand side operand. 13.1
\item \textbf{acc} ( tile < f16 | bf16 | f32 | f64 > ) - The accumulator operand. 13.1
\item \textbf{rounding\_mode} ( RoundingMode ) - The rounding mode for the operation. 13.1
\item \textbf{flush\_to\_zero} ( Flag ) - If set, flushes subnormal inputs and results to sign-preserving zero. 13.1
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The fused multiply-add of the input tiles. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{lhs} , \texttt{rhs} , \texttt{acc} and \texttt{result} must have the same shape and element type ( tile < f16 | bf16 | f32 | f64 > ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The result of the log2 operation. 13.1
...
\end{itemize}
See cuda\_tile.log2\_0 for the full example listing.
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The result of the log operation. 13.1
...
\item \textbf{lhs} ( tile < f16 | bf16 | f32 | f64 > ) - The left hand side operand. 13.1
\item \textbf{rhs} ( tile < f16 | bf16 | f32 | f64 > ) - The right hand side operand. 13.1
\item \textbf{propagate\_nan} ( Flag ) - When set, \texttt{maxf} (or \texttt{minf} ) returns a \texttt{NaN} if either of the two compared elements is \texttt{NaN} . 13.1
\item \textbf{flush\_to\_zero} ( Flag ) - If set, flushes subnormal inputs and results to sign-preserving zero. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The result of the \texttt{maxf} operation. 13.1
...
\end{itemize}
See cuda\_tile.maxf\_0 for the full example listing.
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The minimum of the input tiles. 13.1
...
\end{itemize}
See cuda\_tile.minf\_0 for the full example listing.
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The product of the input tiles. 13.1
...
\begin{itemize}
\item \textbf{source} ( tile < f16 | bf16 | f32 | f64 > ) - The input tile. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The negated floating-point tile. 13.1
...
\end{itemize}
See cuda\_tile.negf\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{source} ( tile < f16 | bf16 | f32 | f64 > ) - The base tile. 13.1
\item \textbf{exponent} ( tile < f16 | bf16 | f32 | f64 > ) - The exponent tile. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The result of the pow operation. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{result} , \texttt{source} and \texttt{exponent} must have the same shape and element type ( tile < f16 | bf16 | f32 | f64 > ).
...
\end{itemize}
See cuda\_tile.pow\_0 for the full example listing.
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The remainder after division. 13.1
...
\begin{itemize}
\item \textbf{source} ( tile < f16 | bf16 | f32 | f64 > ) - The input tile to compute the reciprocal square root of. 13.1
\item \textbf{flush\_to\_zero} ( Flag ) - If set, flushes subnormal inputs and results to sign-preserving zero. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The reciprocal square root of the input tile. 13.1
...
\end{itemize}
See cuda\_tile.rsqrt\_0 for the full example listing.
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The hyperbolic sine of the input tile. 13.1
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The sine of the input tile. 13.1
...
\end{itemize}
See cuda\_tile.sin\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{source} ( tile < f16 | bf16 | f32 | f64 > ) - The input tile to compute the square root of. 13.1
\item \textbf{rounding\_mode} ( RoundingMode ) - The rounding mode for the operation. 13.1
\item \textbf{flush\_to\_zero} ( Flag ) - If set, flushes subnormal inputs and results to sign-preserving zero. 13.1
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The square root of the input tile. 13.1
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The result of the subtraction. 13.1
...
\begin{itemize}
\item \textbf{source} ( tile < f16 | bf16 | f32 | f64 > ) - The input floating-point tile. 13.1
\item \textbf{rounding\_mode} ( RoundingMode ) - The rounding mode for the operation. 13.2


\paragraph{Results}

\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The hyperbolic tangent of the input floating-point tile. 13.1
...
\end{itemize}
See cuda\_tile.tanh\_0 for the full example listing.
...
\item \textbf{result} ( tile < f16 | bf16 | f32 | f64 > ) - The tangent of the input floating-point tile. 13.1
...
\subsection{8.8. Integer}

\end{itemize}
\textbf{Tile IR} contains a set of typed arithmetic operations which implement familiar arithmetic operations on tiles of integers, for floating-point operations see Floating Point .


All operations are implemented in a manner that is efficient for the target architecture and device family. In most common cases this means utilizing the underlying hardware’s native floating-point operations. Due to \textbf{Tile IR} ’s stability guarantees and higher-level programming model some types on some hardware may be emulated, see Stability for more information about the stability guarantees and information about per device behavior.


\subsubsection{8.8.1. Integer Arithmetic}
...
\begin{itemize}
\item \textbf{source} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The input integer tile. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The absolute value of the input tile. 13.1
...
\item The operation is pure and does not perform any memory side effects.
\item \texttt{source} and \texttt{result} must have the same shape.
\item \texttt{source} and \texttt{result} must have the same shape and element type ( tile < i1 | i8 | i16 | i32 | i64 > ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The sum of the input tiles. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{lhs} , \texttt{rhs} and \texttt{result} must have the same shape and element type ( tile < i1 | i8 | i16 | i32 | i64 > ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\item \textbf{rhs} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The right hand side operand. 13.1
\item \textbf{signedness} ( Signedness ) - Interpret integer(s) as \texttt{signed} or \texttt{unsigned} 13.1
\item \textbf{rounding} ( RoundingMode ) - Set the rounding direction (implementing floordiv/ceildiv). 13.1


\paragraph{Results}

\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The result of the division. 13.1
...
\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The result of the maxi operation. 13.1
...
\end{itemize}
See cuda\_tile.maxi\_0 for the full example listing.
...
\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The minimum of the input tiles. 13.1
...
\end{itemize}
See cuda\_tile.mini\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{lhs} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The left hand side input integer tile. 13.1
\item \textbf{rhs} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The right hand side input integer tile. 13.1
\item \textbf{overflow} ( IntegerOverflow ) - The overflow behavior of the operation. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The product of the input tiles. 13.1
...
\begin{itemize}
\item \textbf{x} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The left hand side input integer tile. 13.1
\item \textbf{y} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The right hand side input integer tile. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The most significant bits of the product of the input tiles. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{x} , \texttt{y} and \texttt{result} must have the same shape and element type ( tile < i1 | i8 | i16 | i32 | i64 > ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\end{itemize}
See cuda\_tile.mulhii\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{source} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The input integer tile. 13.1
\item \textbf{overflow} ( IntegerOverflow ) - The overflow behavior of the operation. 13.2


\paragraph{Results}

\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The negated integer tile. 13.1
...
\end{itemize}
See cuda\_tile.negi\_0 for the full example listing.
...
\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The bitwise OR of the input tiles. 13.1
...
\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The remainder after division. 13.1
...
\item The operation may be speculatively executed without side effects.
\item The operation is pure and does not perform any memory side effects.
\item \texttt{result} , \texttt{lhs} and \texttt{rhs} must have the same shape and element type ( tile < i1 | i8 | i16 | i32 | i64 > ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\begin{itemize}
\item \textbf{lhs} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The left hand side operand. 13.1
\item \textbf{rhs} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The right hand side operand (shift amount). 13.1
\item \textbf{overflow} ( IntegerOverflow ) - The overflow behavior of the operation. 13.1


\paragraph{Results}

\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The result of the left shift operation. 13.1
...
\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The result of the right shift operation. 13.1
...
\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The result of the subtraction. 13.1
...
\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The bitwise XOR of the input tiles. 13.1
...
\end{itemize}
See cuda\_tile.xori\_0 for the full example listing.




\subsection{8.9. Bitwise}
...
\item \textbf{result} ( tile < i1 | i8 | i16 | i32 | i64 > ) - The bitwise AND of the input tiles. 13.1
...
\begin{itemize}
\item \textbf{memory\_ordering\_semantics} ( MemoryOrderingSemantics ) - The memory ordering semantics for the atomic operation. 13.1
\item \textbf{memory\_scope} ( MemoryScope ) - The memory scope for the atomic operation. 13.1
\item \textbf{pointers} ( ptr ) - The pointers to the memory locations to perform the atomic compare-and-swap operation on. 13.1
\item \textbf{cmp} ( tile ) - The values to compare against. 13.1
\item \textbf{val} ( tile ) - The values to swap in. 13.1
\item \textbf{mask} ( tile < i1 > ) - The mask for the atomic operation. 13.1
\item \textbf{token} ( token ) - The token for the atomic operation. 13.1


\paragraph{Results}

\item \textbf{result} ( tile ) - The result of the atomic operation. 13.1
\item \textbf{result\_token} ( token ) - The result token of the atomic operation. 13.1
...
\item \texttt{cmp} , \texttt{val} and \texttt{result} must have the same shape and element type ( tile ).
\item The operation must encode variadic operand segment sizes in attributes.
\item The operation’s result type may be inferred from its operands and attributes.
...
\end{itemize}
See cuda\_tile.atomic\_cas\_tko\_0 for the full example listing.
...
\item \textbf{memory\_ordering\_semantics} ( MemoryOrderingSemantics ) - The memory ordering semantics for the load operation. 13.1
\item \textbf{memory\_scope} ( MemoryScope ) - The memory scope for the atomic operation. 13.1
\item \textbf{pointers} ( ptr ) - The pointer tile to perform atomic operation on. 13.1
\item \textbf{mode} ( AtomicRMWMode ) - The atomic operation mode (e.g., add, max, min, etc.). 13.1
\item \textbf{arg} ( tile ) - The value tile to use in the atomic operation. 13.1
\item \textbf{mask} ( tile < i1 > ) - The mask for the load operation. 13.1
\item \textbf{token} ( token ) - The token for the atomic operation. 13.1
...
\begin{itemize}
\item \texttt{arg} and \texttt{result} must have the same shape and element type ( tile ).
\item The operation must encode variadic operand segment sizes in attributes.
\item The operation’s result type may be inferred from its operands and attributes.
...
\end{itemize}
See cuda\_tile.atomic\_rmw\_tko\_0 for the full example listing.




\subsection{8.11. Views}
...
Views are the primary way to interact with global memory in \textbf{Tile IR} . A common pattern is to construct a Tensor View from a pointer with cuda\_tile.make\_tensor\_view and then use the cuda\_tile.load\_view\_tko and cuda\_tile.store\_view\_tko operations to read and write to them. For larger tensors, loading the entire tensor is not efficient and therefore we have a sub-view Partition View which allows a user to tile a \texttt{tensor\_view} .
...
\begin{itemize}
\item \textbf{src} ( view\_type ) - The source view type. 13.1


\paragraph{Results}

\item \textbf{result} ( Variadic < tile < any > > ) - The shape of the index space, each value representing the size of the corresponding dimension. 13.1
...
\end{itemize}
See cuda\_tile.get\_index\_space\_shape\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{src} ( tensor\_view ) - The source tensor view. 13.1


\paragraph{Results}

\item \textbf{result} ( Variadic < tile < any > > ) - The shape of the tensor, each value representing the size of the corresponding dimension. 13.1
...
\end{itemize}
See cuda\_tile.get\_tensor\_shape\_0 for the full example listing.
...
\item \textbf{memory\_ordering\_semantics} ( MemoryOrderingSemantics ) - The memory ordering semantics for the load operation. 13.1
\item \textbf{memory\_scope} ( MemoryScope ) - The memory scope for the atomic operation. 13.1
\item \textbf{view} ( view\_type ) - The view from which the tile will be loaded. 13.1
\item \textbf{index} ( Variadic < tile < any > > ) - The n-dimensional index of the desired element to load from the view. 13.1
\item \textbf{token} ( token ) - The optional token for the load operation. 13.1
\item \textbf{optimization\_hints} ( OptimizationHints ) - Optimization hints for operation 13.1


\paragraph{Results}

\item \textbf{tile} ( tile ) - The loaded tile. 13.1
\item \textbf{result\_token} ( token ) - The result token. 13.1
...
\end{itemize}
See cuda\_tile.load\_view\_tko\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{tensor\_view} ( tensor\_view ) - The source tensor view to create a partition view from. 13.1


\paragraph{Results}

\item \textbf{result} ( partition\_view ) - The created partition view. 13.1


\paragraph{Description}

\end{itemize}
The \texttt{make\_partition\_view} operation creates a partition\_view from a tensor\_view . For more details about partition views see Partition View .
...
The resulting partition view can be loaded from using cuda\_tile.load\_view\_tko and stored to using cuda\_tile.store\_view\_tko .
...
\end{itemize}
See cuda\_tile.make\_partition\_view\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{base} ( tile < ptr > ) - The scalar base pointer to a portion of global memory. 13.1
\item \textbf{dynamicShape} ( Variadic < tile < any > > ) - The array of values representing the shape of the view, may be fully dynamic. 13.1
\item \textbf{dynamicStrides} ( Variadic < tile < any > > ) - The array of values representing the strides of the view, may be fully dynamic. 13.1


\paragraph{Results}

\item \textbf{result} ( tensor\_view ) - The constructed tensor\_view. 13.1


\paragraph{Description}

\end{itemize}
The \texttt{make\_tensor\_view} operation constructs a \texttt{tensor\_view} from a global memory pointer, a dynamic shape and dynamic strides. See Tensor View for more details.
...
\end{itemize}
See cuda\_tile.make\_tensor\_view\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{memory\_ordering\_semantics} ( MemoryOrderingSemantics ) - The memory scope for the store operation. 13.1
\item \textbf{memory\_scope} ( MemoryScope ) - The memory scope for the store operation. 13.1
\item \textbf{tile} ( tile ) - The tile to store. 13.1
\item \textbf{view} ( view\_type ) - The view to store the tile to. 13.1
\item \textbf{index} ( Variadic < tile < any > > ) - The indices of the desired target tile within the view. 13.1
\item \textbf{token} ( token ) - The optional token for operation ordering. 13.1
\item \textbf{optimization\_hints} ( OptimizationHints ) - Optimization hints for operation 13.1
...
\end{itemize}
See cuda\_tile.store\_view\_tko\_0 for the full example listing.




\subsection{8.12. Miscellaneous}
...
\begin{itemize}
\item \textbf{value} ( Any ) - The value to attach the predicate to. 13.1
\item \textbf{predicate} ( AssumePredicate ) - The predicate to attach to the value. 13.1


\paragraph{Results}

\item \textbf{result} ( Any ) - The value with the attached predicate. 13.1
...
See cuda\_tile.assume\_example\_Bounded\_0 for the full example listing.
...
See cuda\_tile.assume\_example\_DivBy\_0 for the full example listing.
...
See cuda\_tile.assume\_example\_DivBy\_1 for the full example listing.
...
See cuda\_tile.assume\_example\_DivBy\_2 for the full example listing.
...
See cuda\_tile.assume\_example\_DivBy\_3 for the full example listing.
...
See cuda\_tile.assume\_example\_DivBy\_4 for the full example listing.
...
See cuda\_tile.assume\_example\_SameElements\_0 for the full example listing.
...
\begin{itemize}
\item \texttt{value} and \texttt{result} must have the same shape and element type ( Any ).
\item The operation’s result type may be inferred from its operands and attributes.
...
\end{itemize}
See cuda\_tile.assume\_0 for the full example listing.
...
\begin{itemize}
\item \textbf{str} ( String ) - The format string. 13.1
\item \textbf{args} ( Variadic < tile > ) - The arguments to format and print. 13.1
\item \textbf{token} ( token ) - The optional token for operation ordering. 13.2


\paragraph{Results}

\item \textbf{result\_token} ( token ) - The result token for synchronization. 13.2
...
\end{itemize}
See cuda\_tile.print\_tko\_0 for the full example listing.


\newpage
\section{9. Debug Info}
...
\subsection{9.1. Location Information}
...
\subsubsection{9.1.1. Location Types}
...
\subsubsection{9.1.4. Trade-offs}
...
\subsubsection{9.2.3. Lexical Block}
...
\subsubsection{9.2.4. Subprogram}
