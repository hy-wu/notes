...
\subsection{在 GPU 上使用tile进行可扩展的数据并行计算}
...
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/introduction.html#section-introduction}{简介} 本节。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/prog_model.html#section-prog-model}{编程模型} 描述了 \textbf{Tile IR} 的编程模型。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/syntax.html#section-syntax}{语法}定义了本规范中使用的 \textbf{Tile IR} 程序的语法。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/bytecode.html#section-bytecode}{二进制格式} 描述了 \textbf{Tile IR} 字节码及其二进制格式。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#section-types}{类型系统}定义了 \textbf{Tile IR} 的类型系统。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#section-semantics}{语义} 描述了 \textbf{Tile IR} 的半形式化语义。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/memory_model.html#section-memory-model}{内存模型} 描述了 \textbf{Tile IR} 内存模型。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#section-operations}{操作} \textbf{Tile IR} 完整操作集的列表。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/debug_info.html#section-debug-info}{调试信息} 描述了 \textbf{Tile IR} 程序的调试信息格式。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/stability.html#section-stability}{稳定性} 描述了 \textbf{Tile IR} 的稳定性保证。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#section-appendix}{附录} 包含相关的参考资料，包括完整程序和参考文献。
\end{enumerate}
...
深度学习重要性的提升，既为用户工作负载引入了更强的规律性，也对这些工作负载的性能交付提出了日益增长的需求。正如\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/introduction.html#section-introduction}{引言}中所讨论的，这催生了以张量核心形式出现的新型专用硬件。
...
\subsubsection{内核输入/输出}

为了说明 \textbf{Tile IR} 的设计，我们将从简单的 hello world 内核转向一个实现一维张量（即向量）加法且具有固定块大小 \texttt{128} 的内核。

本节中展示的所有示例均可在\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#section-appendix-sub-prog}{编程模型示例程序}中找到。
...
\textbf{Tile IR} 中的所有值要么是张量，要么是张量视图（参见\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-views}{张量视图}）。张量是一个 n 维矩形数组，由其秩（维度数量）、形状（每个维度上的大小）及其基本元素类型描述。
...
我们使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-iota}{cuda\_tile.iota}，它构造一个范围张量，从 \texttt{0} 计数到 \texttt{n - 1}，形成一个大小为 \texttt{n} 的向量。
...
到目前为止，我们只研究了为单个tile块编写的内核。\textbf{Tile IR} 允许将tile块分组为\textbf{tile网格}，类似于 CUDA C++，使用户能够启动并行执行的tile块集合。与 PTX 一样，tile内核在tile块坐标上隐式参数化（可通过 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-get-tile-block-id}{cuda\_tile.get\_tile\_block\_id} 查询），并且可以是一维、二维或三维的。
...
每个tile内核都可以使用一维、二维或三维网格启动。每个tile块可以通过\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-get-tile-block-id}{cuda\_tile.get\_tile\_block\_id}查询其在网格中的位置，并通过改变参数来查询第一、第二或第三维度的索引。

tile内核也可以使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-get-num-tile-blocks}{cuda\_tile.get\_num\_tile\_blocks} 以类似的方式观察网格维度。
...
我们随后加载输入参数，计算MMA操作（浮点数计算参见\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-mmaf}{cuda\_tile.mmaf}，整数计算参见\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-mmai}{cuda\_tile.mmai}）以计算输出图块，然后将其存储。
...
我们还使用\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-iota}{cuda\_tile.iota}为归约维度再次创建了一个从\texttt{(0, 63)}的共享范围。
...
\caption{全局内存视图}
\end{figure}
指向A的基指针指向一个位于全局内存中的分配。\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/prog_model.html#id1}{\#}
...
为了简化\textbf{Tile IR}的编程模型并提高用户程序的效率，\textbf{Tile IR}还引入了一种称为张量视图的结构化指针类型。张量视图可以通过\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-make-tensor-view}{cuda\_tile.make\_tensor\_view}从原始指针构建，它将形状和步长信息附加到指针上，有效地表示一个指向张量的类型化指针。


在构建张量视图时，我们使用静态或动态的形状和步幅信息将原始指针转换为张量，这是通过 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-make-tensor-view}{cuda\_tile.make\_tensor\_view} 完成的。此操作将形状和步幅信息附加到指针上，有效地将类型化指针转换为张量。
...
\caption{全局内存视图}
\end{figure}
指向A的基指针指向一个位于全局内存中的分配。\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/prog_model.html#id2}{\#}
...
一旦你构建了张量视图，我们就可以利用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-make-partition-view}{cuda\_tile.make\_partition\_view} 来对底层张量进行分块处理。
...
\end{figure}
指向A的基指针指向一个位于全局内存中的分配。 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/prog_model.html#id3}{\#}
...
\subsubsection{使用视图进行向量加法}

我们现在可以实现一个更复杂的向量操作，即SAXPY或``单精度A·X加Y''（一种常见的

BLAS操作）。我们可以使用\texttt{tensor view}和\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-make-partition-view}{cuda\_tile.make\_partition\_view}来为任意大小的向量实现此操作。
...
我们使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-make-partition-view}{cuda\_tile.make\_partition\_view} 为 \texttt{\%x} 和 \texttt{\%y} 创建视图，这些视图表示一个 \texttt{(M/128 x N/256)} 的tile张量。每个tile的大小由分区类型的 tile 参数指定。
...
\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko} 允许我们为 \%X 和 \%Y 加载由 \texttt{\%view[\%x, \%y]} 指定的图块。
...
\textbf{Tile IR} 编译器可以在已知底层指针和步幅的对齐方式时，优化 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{Tensor View} 的内存加载和存储。如果这些信息是静态已知的，我们可以直接推断对齐方式；但如果它们是动态的（如此例所示），我们可以使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-assume}{cuda\_tile.assume} 操作来告知编译器指针已正确对齐。
...
我们为每个张量创建一个\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{分区视图}。首先是张量A：
...
然后我们使用\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-get-tile-block-id}{cuda\_tile.get\_tile\_block\_id}读取图块网格坐标。
...
由于\texttt{K}维度是动态的，我们本可以自行计算归约索引空间的大小，但\textbf{Tile IR}提供了一个操作来为我们完成这项工作。\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-get-index-space-shape}{cuda\_tile.get\_index\_space\_shape} 可用于获取索引空间的大小，从而确定归约循环的规模。
...
现在，我们可以使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-for}{cuda\_tile.for} 循环来遍历图块块。


\texttt{for} 循环是 \textbf{Tile IR} 中的一种结构化控制流操作，它使循环变量在 \texttt{(起始值, 结束值, 步长)} 定义的范围内逐步遍历，并为范围内的每个值执行循环体。

iter\_values 是循环体的循环传递变量，它们在循环头中被初始化，并通过使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-continue}{cuda\_tile.continue} 操作产生它们来在每次迭代中更新。
...
\end{itemize}
单个操作的编码在\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#section-operations}{操作}中有详细描述。
...
\end{itemize}
debug 部分存储了序列化的调试信息（有关调试信息的更多详情，请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/debug_info.html#section-debug-info}{调试信息}）。
...
（即不支持嵌套指针类型）。关于不同指针类型之间或与整数之间的转换详情，请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-bitcast}{cuda\_tile.bitcast}。
...
我们使用 \texttt{tile<MxNxKxE>} 作为瓦片类型的语法，其中 \texttt{M}、\texttt{N}、\texttt{K} 是整数，\texttt{E} 是元素类型。更多详细信息请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/syntax.html#section-syntax}{语法}。
...
> 指针张量的形状定义了加载或存储的值的形状。它并不暗示位置本身有任何结构。例如，张量中两个连续的元素在内存中可能不是连续的位置。更进一步，同一个位置可能在一个指针张量中出现多次。有关影响的讨论，请参见\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/memory_model.html#section-memory-model}{内存模型}。
...
$$elem_{[i_0,\ldots,i_n]}=baseptr+\sum_{m=0}^n i_m * s_m$$
其中 $s_m$ 是张量视图的步长因子，而 $baseptr$ 是全局内存中的起始地址。


除了瓦片类型外，张量视图还支持形状向量中的动态范围。我们使用 \texttt{?} 字符来表示这些动态范围。动态范围在运行时通过 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-make-tensor-view}{cuda\_tile.make\_tensor\_view} 操作构造视图类型时绑定到具体值。


一个动态形状的张量视图不能直接用于访问内存。它首先需要被分割成静态大小的块。


张量视图通过 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-make-tensor-view}{cuda\_tile.make\_tensor\_view} 操作构建，并包含以下组成部分：
...
子视图描述了从索引空间到从张量视图加载的静态大小图块空间的映射。它们定义了在从张量视图访问元素时，由 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko} 执行的必要索引计算。子视图的大小由其索引空间的基数定义。
...
分区视图结构是使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-make-partition-view}{cuda\_tile.make\_partition\_view} 构造函数创建的。
...
$$location[i_0,\ldots,i_n] = baseptr + \sum_{m=0}^{n} I_m \cdot \mathrm{ceildiv}(S_m, T_m) \cdot st_m$$
...
它\textbf{并不}试图形式化所有可能被公理化表述或小步操作语义所允许的行为。要了解关于该语言及其核心概念更为非正式的介绍，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/prog_model.html#section-prog-model}{编程模型}部分。


我们首先介绍抽象机器状态和语言定义，然后描述各个内核和程序的语义。


然后我们也会讨论广泛操作类别的语义，至于各个操作及其行为的更详细描述，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#section-operations}{操作}以获取完整列表。
...
\begin{itemize}
\item 一个格式良好的模块 $\mathcal{Mod}$，它存储一个或多个项目，将在下面的\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#sub-sec-modules}{模块}部分进行讨论。
...
\begin{itemize}
\item 一个\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#sub-sec-tile-global}{全局变量定义}
\item 一个\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#sub-sec-tile-kernel}{瓦片内核}
\item 一个\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#sub-sec-tile-function}{瓦片函数}
...
全局变量使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-global}{cuda\_tile.global} 操作进行声明。

全局变量必须在声明时初始化，并且只会被初始化一次。


全局变量必须包含一个\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{Tile类型}的值。


可以使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-get-global}{cuda\_tile.get\_global} 操作来获取一个指针，通过该指针可以读写全局变量，从而修改全局变量。
...
一个瓦片内核可以通过 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-get-tile-block-id}{cuda\_tile.get\_tile\_block\_id} 查询其网格坐标，坐标可以是一维、二维或三维，具体取决于启动内核时使用的网格。内核也可以通过 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-get-num-tile-blocks}{cuda\_tile.get\_num\_tile\_blocks} 查询每个维度上的瓦片块总数。
...
一个瓦片函数有 N 个形式参数并产生 M 个返回值。参数的类型可以是\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#section-types}{类型系统}中描述的有效类型之一。
...
\item 程序根据\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#section-types}{类型系统}中指定的规则进行类型检查，且运算符类型签名在\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#section-operations}{操作}中指定。
...
\textbf{Tile IR} 拥有一小组类型，如\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#section-types}{类型系统}中所述，但我们只有两种类型的值。


\begin{itemize}
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{Pointers}，表示一个内存地址。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{Tiles} ，或标量的 N 维数组。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-views}{Views}，表示内存的结构化视图。
...
指针必须对齐到它们所指向的基础数据类型的大小，具体编码和分配布局信息请参见\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#element-type-encoding}{元素类型编码}。
...
输入指针值所指向的分配，以及由此扩展的视图（参见\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-views}{下文}），必须符合指定的数据布局。
...
顺便提一下，数据类型编码与\href{https://github.com/dmlc/dlpack}{DLPack}兼容，这是大多数深度学习框架和数组库采用的标准。我们为每种数据类型提供了等效 PyTorch 和 NumPy 编码。


对于NumPy低精度类型，我们通过\href{https://github.com/jax-ml/ml_dtypes}{ml\_dtypes}库提供了等效的标准低精度NumPy数据类型集合。
...
\textbf{Tile IR} 提供了一组视图类型，如 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-views}{Tensor View} 中所述。


视图通过为指针添加额外数据，提供了对内存的结构化视图。

视图拥有自己的一组内存操作\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko}和\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko}，这些操作的行为根据视图类型的不同而变化，并利用了额外的元数据。由于视图并非单一类型，而是一个类型家族，每个具体的视图类型都有其自身的值表示形式。


我们的一阶视图类型是 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{Tensor View}。张量视图值在逻辑上是一个元组 $(ptr, \vec{shape}, \vec{strides}, \vec{dimgroups})$，加载和存储操作作用于整个张量视图。该视图的索引空间是零秩的（即索引空间中只有一个元素）。


我们的主要二阶（或``子视图''）视图类型是\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{分区视图}。


分区视图类型 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{分区视图} 是一种子视图类型，它表示被分割成图块的张量视图。

分区视图值在逻辑上是一个元组（$tensor\_view, \vec{tile\_size}$），其中加载和存储操作在给定大小的图块值上进行。此视图的索引空间为：


$$indexSpace[i] = \mathrm{tensorViewShape}[\mathrm{dimGroups}[i]] / \mathrm{tileSize}[i]$$
例如，一个形状为 $(1024, 1024)$ 的张量视图可以被划分为一个 $64 \times 32$ 的瓦片网格，从而得到一个形状为 $(16, 32)$ 的分区视图。有关不同类型视图的更多详细信息，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#section-types}{类型系统}。
...
$$\mathrm{tile\_grid}(\vec{i})=\begin{cases}
...
值仅通过内存操作从全局内存中获取或持久化到全局内存中（参见\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-memory}{内存}）。
...
> 全局内存\textbf{是}与CUDA程序使用的相同全局设备内存，并在\href{https://docs.nvidia.com/cuda/parallel-thread-execution/#state-spaces}{PTX规范}中有所描述。


在\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/memory_model.html#section-memory-model}{内存模型}中详细阐述了\textbf{Tile IR}内存模型的复杂特性。
...
\end{itemize}
\textbf{Tile IR} 程序的执行始于内核启动。内核启动 API 对所有 CUDA 内核都是统一的，并在 \href{https://docs.nvidia.com/cuda/cuda-runtime-api/index.html}{CUDA 运行时 API} 中规定。
...
有关更详细的示例程序及其执行说明，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/prog_model.html#section-prog-model}{编程模型}或\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#section-appendix}{附录}。
...
关于内存模型和内存操作的详细讨论，请参见\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/memory_model.html#section-memory-model}{内存模型}。



\subsubsection{终止}

当瓦片块的函数体到达最终语句时，瓦片块将终止。瓦片内核必须通过返回操作 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-return}{cuda\_tile.return} 来终止，该操作标志着执行结束。
...
\subsection{\textbf{Tile IR} 内存模型中的风险与直觉}

\subsubsection{从未来读取的令牌顺序操作}

在不使用令牌约束的令牌有序操作中，存储缓冲和加载缓冲行为是可见的。



\subsubsection{单个瓦片块线程内的竞争风险}
...
\subsubsection{关于如何使用令牌排序的一些直观理解}
...
例如，要使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-offset}{cuda\_tile.offset} 操作向指针添加偏移瓦片，必须使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-reshape}{cuda\_tile.reshape} 或 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-broadcast}{cuda\_tile.broadcast} 操作对指针和偏移进行重塑或广播，使其具有相同的形状。



\subsubsection{独立的浮点与整数运算}

数值运算根据舍入模式、\texttt{NaN} 处理和快速数学等标志的差异，被分为整数和浮点类型。


例如，\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-addf}{cuda\_tile.addf} 操作支持舍入属性，但 addi 操作不支持。



\subsubsection{显式溢出标注}

一些操作，例如 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-addi}{cuda\_tile.addi} 支持显式的溢出注解，用以表达该操作预期的溢出行为。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 要广播的 tile。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 广播后的 tile。$\boxed{13.1}$
...
\begin{itemize}
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}）- 右侧操作数。$\boxed{13.1}$
\item \textbf{dim} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64} ) - 沿此维度进行拼接。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 拼接后的结果图块。$\boxed{13.1}$
...
$$\operatorname{cat}(x, y, dim_{cat})[\vec{i}] = \begin{cases}
...
\item \texttt{lhs}、\texttt{rhs} 和 \texttt{result} 必须具有相同的元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}）。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-cat-0}{cuda\_tile.cat\_0}。
...
\begin{itemize}
\item \textbf{comparison\_predicate} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-cmpf-comparisonpredicate-attr}{ComparisonPredicate} ) - 比较谓词。$\boxed{13.1}$
\item \textbf{comparison\_ordering} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-cmpf-comparisonordering-attr}{ComparisonOrdering} ) - 比较顺序。$\boxed{13.1}$
\item \textbf{lhs} (\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 右侧操作数。$\boxed{$\textbackslash{}boxed\{13.1\}$}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{>}) - 比较的结果。$\boxed{13.1}$
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-cmpf-0}{cuda\_tile.cmpf\_0} 获取完整示例列表。
...
\begin{itemize}
\item \textbf{comparison\_predicate} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-cmpi-comparisonpredicate-attr}{ComparisonPredicate} ) - 比较谓词。$\boxed{13.1}$
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{有符号性} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-cmpi-signedness-attr}{有符号性} ) - 将整数解释为 \texttt{有符号} 或 \texttt{无符号} $\boxed{13.1}$
...
\end{itemize}
请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-cmpi-0}{cuda\_tile.cmpi\_0}获取完整的示例列表。
...
\begin{itemize}
\item \textbf{value} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-denseconstant}{DenseConstant} ) - 要创建的常量值。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e4m3fn}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e5m2}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32}\texttt{>}) - 常量图块。$\boxed{13.1}$
...
\item 该操作可以无副作用地推测执行。
\item 该操作是纯粹的，不会产生任何内存副作用。
\item \texttt{value} 和 \texttt{result} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-denseconstant}{DenseConstant}）。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-constant-0}{cuda\_tile.constant\_0} 获取完整的示例列表。
...
\begin{itemize}
\item \textbf{sym\_name} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-symbol}{Symbol} ) - 函数名称。$\boxed{13.1}$
\item \textbf{function\_type} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-type}{类型} ) - 函数的类型。$\boxed{13.1}$
\item \textbf{arg\_attrs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-attributes}{属性} ) - 函数的参数属性：TileIR 目前不支持其中任何属性。$\boxed{13.1}$
\item \textbf{res\_attrs}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-attributes}{属性}）- 函数的结果属性：TileIR 目前不支持其中任何属性。$\boxed{13.1}$
\item \textbf{optimization\_hints} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-entry-optimizationhints-attr}{OptimizationHints} ) - 编译器架构特定的优化提示 $\boxed{13.1}$
...
有关瓦片内核的详细语义，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#sub-sec-tile-kernel}{瓦片内核}。
...
\begin{itemize}
\item \texttt{num\_cta\_in\_cga} - 为 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-entry}{cuda\_tile.entry} 建议 CGA 中的 CTA 数量（必须是小于或等于 16 的 2 的幂次）。
\item \texttt{allow\_tma} - 建议是否对 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko} 使用 TMA。
\item \texttt{latency} - 为 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko} 提供的延迟提示。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 从中提取数据的源 tile。$\boxed{13.1}$
\item \textbf{indices} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{瓦片}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32} > > ) - 要提取的切片索引。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 提取出的子瓦片。$\boxed{13.1}$
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-extract-0}{cuda\_tile.extract\_0} 获取完整示例列表。
...
\begin{itemize}
\item \textbf{name} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-symbol}{Symbol} ) - 全局变量的名称。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr}\texttt{>}) - get\_global 操作的结果。$\boxed{13.1}$


\paragraph{描述}

\end{itemize}
\texttt{get\_global} 操作返回指向指定 \texttt{global} 变量的指针。全局变量是一种静态全局内存分配形式，可以使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-global}{cuda\_tile.global} 操作来声明。


返回指针的元素类型将与声明的全局变量的元素类型相同。


有关全局变量的详细语义，请参见\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#sub-sec-tile-global}{全局变量}。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-get-global-0}{cuda\_tile.get\_global\_0}。
...
\begin{itemize}
\item \textbf{gridSize\_x} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{>}) - 维度 \texttt{x} 中的瓦片块数量。$\boxed{13.1}$
\item \textbf{gridSize\_y} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{>}) - 维度 \texttt{y} 中的瓦片块数量。$\boxed{13.1}$
\item \textbf{gridSize\_z} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{>}) - 维度 \texttt{z} 中的瓦片块数量。$\boxed{13.1}$
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-get-num-tile-blocks-0}{cuda\_tile.get\_num\_tile\_blocks\_0} 获取完整的示例列表。
...
\begin{itemize}
\item \textbf{blockId\_x} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{>}) - 维度 \texttt{x} 的图块块ID。$\boxed{13.1}$
\item \textbf{blockId\_y} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{>}) - 维度 \texttt{y} 的瓦片块ID。$\boxed{13.1}$
\item \textbf{blockId\_z}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32} >）- 维度 \texttt{z} 的瓦片块ID。$\boxed{13.1}$
...
\begin{itemize}
\item \textbf{sym\_name} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-symbol}{Symbol} ) - 全局变量的名称。$\boxed{13.1}$
\item \textbf{value} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-denseconstant}{DenseConstant} ) - 用于初始化分配的值。$\boxed{13.1}$
\item \textbf{alignment} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64} ) - 缓冲区的对齐方式。$\boxed{13.1}$
...
\texttt{global} 操作在全局内存中静态分配一个可变的一维位置，并使用 \texttt{value} 对其进行初始化。分配的初始化在 \href{https://docs.nvidia.com/cuda/cuda-driver-api/group__CUDA__TYPES.html#group__CUDA__TYPES_1g9e4ef4dcfba4662b2299acb8d049a1ef}{CUDA 模块} 加载时执行。分配的生存期与模块的生存期相同。


分配的内存可以通过以下方式读取或写入：首先使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-get-global}{cuda\_tile.get\_global} 获取指向该内存的指针，然后使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-ptr-tko}{cuda\_tile.load\_ptr\_tko} 读取，或使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-ptr-tko}{cuda\_tile.store\_ptr\_tko} 写入。


初始值按线性顺序存储在内存中，因此由\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-get-global}{cuda\_tile.get\_global}返回的指针指向第一个元素，通过偏移x可以加载位置x处的元素。
...
有关全局变量的更详细语义，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#sub-sec-tile-global}{全局变量}。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-global-0}{cuda\_tile.global\_0}。
...
\begin{itemize}
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - iota操作的结果。$\boxed{13.1}$
...
\begin{itemize}
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32} > > ) - 左侧矩阵操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32} > > ) - 右侧矩阵操作数。$\boxed{13.1}$
\item \textbf{acc} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 累加器矩阵操作数。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 乘加运算后的结果矩阵。$\boxed{13.1}$
...
所有操作数的类型必须是受支持的组合（参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#table-cuda-tile-mmaf-0}{mmaf 支持的数据类型}）。
...
\item 该操作是纯粹的，不会产生任何内存副作用。
\item \texttt{acc} 和 \texttt{result} 必须具有相同的形状和元素类型（ \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64} > ）。
\item \texttt{lhs} 和 \texttt{rhs} 必须具有相同的元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32} > >）。
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-mmaf-0}{cuda\_tile.mmaf\_0} 获取完整示例列表。
...
\begin{itemize}
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{>}) - 左侧矩阵操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{>}) - 右侧矩阵操作数。$\boxed{13.1}$
\item \textbf{acc} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{>}) - 累加器矩阵操作数。$\boxed{13.1}$
\item \textbf{signedness\_lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-mmai-signedness-attr}{Signedness} ) - \texttt{lhs} 操作数的有符号性。$\boxed{13.1}$
\item \textbf{signedness\_rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-mmai-signedness-attr}{Signedness} ) - \texttt{rhs} 操作数的有符号性。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{>}) - 乘加运算后的结果矩阵。$\boxed{13.1}$
...
\item \texttt{acc} 和 \texttt{result} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32} >）。
\item \texttt{lhs} 和 \texttt{rhs} 必须具有相同的元素类型（ \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8} > ）。
\item \texttt{lhs}、\texttt{rhs} 和 \texttt{acc} 必须具有相同的秩。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-mmai-0}{cuda\_tile.mmai\_0} 获取完整的示例列表。
...
\begin{itemize}
\item \textbf{sym\_name} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-symbol}{Symbol} ) - 模块的名称。$\boxed{13.1}$
...
有关模块语义的详细说明以及每种项目类型的完整定义，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#sub-sec-modules}{模块}。
...
\begin{itemize}
\item \textbf{ptr}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr}）- 要推进的基础指针图块。$\boxed{13.1}$
\item \textbf{offset} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 要加到指针上的偏移量图块。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr} ) - 前进后得到的结果指针瓦片。$\boxed{13.1}$
...
\item \texttt{result} 和 \texttt{ptr} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr}）。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e4m3fn}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e5m2}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32}\texttt{>}) - 输入图块。13.2


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{>}) - 打包后的 tile。13.2
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-pack-0}{cuda\_tile.pack\_0} 获取完整示例列表。
...
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-pack-1}{cuda\_tile.pack\_1}。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 输入瓦片。$\boxed{13.1}$
\item \textbf{permutation} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-array}{Array}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{>}) - 维度的排列。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 置换后的 tile。$\boxed{13.1}$
...
\item 该操作是纯粹的，不会产生任何内存副作用。
\item \texttt{source} 和 \texttt{result} 必须具有相同的元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}）。
\item \texttt{source} and \texttt{result} must have the same rank.
...
\end{itemize}
请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-permute-0}{cuda\_tile.permute\_0}获取完整示例列表。
...
\begin{itemize}
\item \textbf{操作数}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{瓦片} >）- 要归约的瓦片集合。$\boxed{13.1}$
\item \textbf{dim} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32} ) - 执行归约操作的维度索引。$\boxed{13.1}$
\item \textbf{identities} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-array}{Array} ) - 每个操作数的归约恒等值。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{results} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{>}) - 归约后的 tile 集合。$\boxed{13.1}$
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-reduce-0}{cuda\_tile.reduce\_0} 查看完整的示例代码清单。
...
请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-reduce-1}{cuda\_tile.reduce\_1}获取完整示例列表。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 要重塑形状的源 tile。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 重塑后的 tile。$\boxed{13.1}$
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-reshape-0}{cuda\_tile.reshape\_0}。
...
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-reshape-1}{cuda\_tile.reshape\_1}。
...
\begin{itemize}
\item \textbf{操作数}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变数量}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{瓦片} >）- 要扫描的一组瓦片。$\boxed{13.1}$
\item \textbf{dim} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32} ) - 沿其进行扫描的维度索引。$\boxed{13.1}$
\item \textbf{reverse} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-bool}{bool} ) - 是否以相反顺序进行扫描。$\boxed{13.1}$
\item \textbf{identities} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-array}{Array} ) - 扫描操作的恒等值。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{results} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{>}) - 扫描操作产生的tile结果。$\boxed{13.1}$
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-scan-0}{cuda\_tile.scan\_0}。
...
\begin{itemize}
\item \textbf{cond} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{>}) - 条件瓦片。$\boxed{13.1}$
\item \textbf{val\_if\_true} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 条件为真时的值 tile。$\boxed{13.1}$
\item \textbf{val\_if\_false} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 条件为假时的值 tile。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 所选值的tile。$\boxed{13.1}$
...
\item 该操作可以在没有副作用的情况下被推测性地执行。
\item 该操作是纯粹的，不会产生任何内存副作用。
\item \texttt{val\_if\_true}、\texttt{val\_if\_false} 和 \texttt{result} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}）。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{>}) - 输入的 i8 tile。13.2


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e4m3fn}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e5m2}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32}\texttt{>}) - 输出的解包后的 tile。13.2
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-unpack-0}{cuda\_tile.unpack\_0}。
...
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-unpack-1}{cuda\_tile.unpack\_1}。
...
\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-bitcast}{cuda\_tile.bitcast} 保留输入内容但允许更改元素类型，\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-exti}{cuda\_tile.exti} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-trunci}{cuda\_tile.trunci} 改变整数图块的宽度，\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-ftoi}{cuda\_tile.ftoi} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-itof}{cuda\_tile.itof} 将浮点图块转换为整数图块，反之亦然，而 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-ftof}{cuda\_tile.ftof} 在不同浮点类型之间进行转换。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e4m3fn}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e5m2}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32}\texttt{>}) - 要转换的源 tile。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e4m3fn}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e5m2}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32}\texttt{>}) - 转换后的 tile。$\boxed{13.1}$
...
指针类型必须使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-ptr-to-int}{cuda\_tile.ptr\_to\_int} 或 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-int-to-ptr}{cuda\_tile.int\_to\_ptr} 替代。
...
\begin{itemize}
\item \textbf{from} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 要扩展的输入整数 tile。$\boxed{13.1}$
\item \textbf{有符号性}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-exti-signedness-attr}{Signedness}）- 将整数解释为 \texttt{有符号} 或 \texttt{无符号} $\boxed{13.1}$


\paragraph{结果}

\item \textbf{to} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 扩展整数瓦片。$\boxed{13.1}$
...
\begin{itemize}
\item \textbf{from} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e4m3fn}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e5m2}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32}\texttt{>}) - 输入的浮点 tile。$\boxed{13.1}$
\item \textbf{rounding\_mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-ftof-roundingmode-attr}{RoundingMode} ) - 该运算的舍入模式。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{to} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e4m3fn}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e5m2}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32}\texttt{>}) - 结果浮点 tile。$\boxed{13.1}$
...
\begin{itemize}
\item \textbf{from} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e4m3fn}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e5m2}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32}\texttt{>}) - 输入的浮点型 tile。$\boxed{13.1}$
\item \textbf{有符号性}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-ftoi-signedness-attr}{Signedness}）- 将整数解释为 \texttt{有符号} 或 \texttt{无符号} $\boxed{13.1}$
\item \textbf{rounding\_mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-ftoi-roundingmode-attr}{RoundingMode} ) - 该操作的舍入模式。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{to} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 结果整数 tile。$\boxed{13.1}$
...
与保留位数的\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-bitcast}{cuda\_tile.bitcast}不同，此操作会保留图块的数值，并向零舍入到所提供类型的最近整数。
...
\begin{itemize}
\item \textbf{from} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入的整数瓦片。$\boxed{13.1}$
\item \textbf{有符号性}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-itof-signedness-attr}{有符号性}）- 将整数解释为 \texttt{有符号} 或 \texttt{无符号} $\boxed{13.1}$
\item \textbf{rounding\_mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-itof-roundingmode-attr}{RoundingMode} ) - 该操作的舍入模式。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{to} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e4m3fn}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e5m2}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32}\texttt{>}) - 转换后的浮点型 tile。$\boxed{13.1}$
...
与 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-bitcast}{cuda\_tile.bitcast} 不同，此操作会保留图块的数值，并将其舍入到所提供类型的最接近的浮点数。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入的整数图块。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr} ) - 输出的指针图块。$\boxed{13.1}$
...
此操作的逆运算是 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-ptr-to-int}{cuda\_tile.ptr\_to\_int}。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr} ) - 输入的指针图块。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 整数输出瓦片。$\boxed{13.1}$
...
此操作的逆运算是 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-int-to-ptr}{cuda\_tile.int\_to\_ptr}。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr} ) - 具有源指针元素类型的 Tile。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr} ) - 具有目标指针元素类型的 Tile。$\boxed{13.1}$
...
为了执行这些转换，请使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-ptr-to-int}{cuda\_tile.ptr\_to\_int} 或 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-int-to-ptr}{cuda\_tile.int\_to\_ptr}。
...
\begin{itemize}
\item \textbf{from} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 要截断的输入整数 tile。$\boxed{13.1}$
\item \textbf{overflow} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-trunci-integeroverflow-attr}{IntegerOverflow} ) - 操作的溢出行为。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{to} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 截断的整数瓦片。$\boxed{13.1}$
...
这些操作的设计风格参考了 \href{https://mlir.llvm.org/docs/Dialects/ControlFlow/}{MLIR 控制流方言}。


一个显著的区别在于我们允许控制流操作的嵌套，例如 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-if}{cuda\_tile.if} 可以出现在 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-loop}{cuda\_tile.loop} 或 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-for}{cuda\_tile.for} 内部。
...
\begin{itemize}
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-if}{cuda\_tile.if} 用于实现条件分支。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-loop}{cuda\_tile.loop} 实现了具有任意退出条件的循环。
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-for}{cuda\_tile.for} 实现了一个基于范围的循环，具有固定的迭代次数。
...
\begin{itemize}
\item \textbf{condition} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{>}) - 要检查的条件瓦片。$\boxed{13.1}$
\item \textbf{message} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-string}{String} ) - 断言失败时显示的错误信息。$\boxed{13.1}$
...
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-assert-0}{cuda\_tile.assert\_0} 查看完整示例列表。
...
\begin{itemize}
\item \textbf{操作数}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{任意类型} >）- 在循环终止时返回给父循环的操作数。$\boxed{13.1}$
...
\texttt{break} 操作是 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-loop}{cuda\_tile.loop} 的一个终止操作。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-break-0}{cuda\_tile.break\_0}。
...
\begin{itemize}
\item \textbf{操作数}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{任意类型} >）- 要传递给父循环的值。$\boxed{13.1}$
...
\texttt{continue} 操作表示一个块终止符，它将控制权返回给循环操作，例如 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-for}{cuda\_tile.for} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-loop}{cuda\_tile.loop}。该操作在终止时可以向父循环传递任意数量的 \texttt{\$operands}。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-continue-0}{cuda\_tile.continue\_0}。
...
\begin{itemize}
\item \textbf{lowerBound} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{any}\texttt{>}) - 循环的下界。$\boxed{13.1}$
\item \textbf{upperBound} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{any}\texttt{>}) - 循环的上界。$\boxed{13.1}$
\item \textbf{step} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{any}\texttt{>}) - 循环的步长。$\boxed{13.1}$
\item \textbf{initValues} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{Variadic}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{Any}\texttt{>}) - 循环传递值的初始值。$\boxed{13.1}$
\item \textbf{unsignedCmp}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{Flag}）- 如果存在，则使用无符号整数比较进行循环终止。13.2


\paragraph{结果}

\item \textbf{resultValues} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{任意类型}\texttt{>}) - 循环结束后循环携带变量的值。$\boxed{13.1}$
...
循环体必须以一个\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-continue}{cuda\_tile.continue}结束，该操作为每个循环携带的变量生成下一次迭代的值。
...
\begin{itemize}
\item 循环携带的变量不能是\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{tensor\_view}或视图类型。
\item \texttt{for} 操作无法提前终止，必须以 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-continue}{cuda\_tile.continue} 结束。


\paragraph{约束条件}

\item 当栈分配自动释放时，操作必须定义作用域。
\item \texttt{lowerBound}、\texttt{upperBound} 和 \texttt{step} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{any} >）。
\item \texttt{initValues} 和 \texttt{resultValues} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{Variadic}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{Any} >）。
\item 该操作必须提供自定义的解析和打印方法。
\item 该操作有且仅在该区域的操作有效时才有效。
\item 每个提供的区域必须恰好包含一个区块。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-for-0}{cuda\_tile.for\_0}。
...
\begin{itemize}
\item \textbf{condition} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{>}) - if 操作的条件。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{results} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{任意类型}\texttt{>}) - if操作的结果。$\boxed{13.1}$
...
\texttt{if} 操作可以通过在每个分支中使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-yield}{cuda\_tile.yield} 产生值来生成结果。
...
\begin{itemize}
\item \texttt{if} 的结果不能是 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{tensor\_view} 或视图类型。
...
\end{itemize}
请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-if-0}{cuda\_tile.if\_0}获取完整示例列表。
...
\begin{itemize}
\item \textbf{initValues} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{Variadic}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{Any}\texttt{>}) - 循环的初始值。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{resultValues} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{任意类型}\texttt{>}) - 循环的结果值。$\boxed{13.1}$


\paragraph{描述}

\end{itemize}
\texttt{loop} 操作表示一个非结构化的无限循环，该循环会一直执行，直到遇到 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-break}{cuda\_tile.break}。
...
循环将执行循环体，直到动态执行了\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-break}{cuda\_tile.break}。
...
\begin{itemize}
\item 一个 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-continue}{cuda\_tile.continue}，它为每个循环携带的变量生成下一次迭代的值。
\item 一个终止循环并产生最终循环携带值的 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-break}{cuda\_tile.break}。
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-loop-0}{cuda\_tile.loop\_0} 获取完整示例列表。
...
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-loop-1}{cuda\_tile.loop\_1} 获取完整示例列表。
...
请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-loop-2}{cuda\_tile.loop\_2}获取完整示例列表。
...
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-loop-3}{cuda\_tile.loop\_3} 查看完整示例列表。
...
\begin{itemize}
\item \textbf{操作数}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{任意类型} >）- 要返回的值。$\boxed{13.1}$
...
\begin{itemize}
\item \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-entry}{cuda\_tile.entry} 操作不产生返回值，因此可以通过不带操作数调用该操作来使用 \texttt{return} 终止内核的执行
...
\end{itemize}
完整示例代码请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-return-0}{cuda\_tile.return\_0}。
...
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-return-1}{cuda\_tile.return\_1} 获取完整的示例列表。
...
\begin{itemize}
\item \textbf{操作数}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变数量}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{任意类型} >）- 要传递给父操作的操作数。$\boxed{13.1}$
...
与标准的MLIR控制流方言不同，\texttt{yield}不用于循环控制流，有关循环控制流，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-break}{cuda\_tile.break}和\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-continue}{cuda\_tile.continue}。
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-yield-0}{cuda\_tile.yield\_0} 获取完整示例列表。
...
\begin{itemize}
\item 基于指针的内存操作，例如 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-ptr-tko}{cuda\_tile.load\_ptr\_tko} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-ptr-tko}{cuda\_tile.store\_ptr\_tko}，用于从全局内存加载图块以及将图块存储到全局内存。
\item 基于视图的内存操作，例如 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko}，它们分别从视图加载图块以及将图块存储到视图。
\item 原子内存操作，例如 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-atomic-rmw-tko}{cuda\_tile.atomic\_rmw\_tko} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-atomic-cas-tko}{cuda\_tile.atomic\_cas\_tko}，它们对全局内存执行原子操作。

\end{itemize}
目前所有内存操作都是令牌有序的；任意一对内存操作之间的顺序是未定义的，除非通过令牌连接。有关令牌有序操作的更多讨论，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/memory_model.html#section-memory-model}{内存模型}。
...
当使用填充视图或掩码时，关于构成越界的规则会被修改，有关特定类型的更多详细信息，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#section-types}{类型系统}。
...
\begin{itemize}
\item \textbf{tokens} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{令牌}\texttt{>}) - 要连接的输入令牌。$\boxed{13.1}$


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 合并后的令牌。$\boxed{13.1}$
...
\begin{itemize}
\item \textbf{memory\_ordering\_semantics} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-load-ptr-tko-memoryorderingsemantics-attr}{MemoryOrderingSemantics} ) - 加载操作的内存排序语义。$\boxed{13.1}$
\item \textbf{memory\_scope} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-load-ptr-tko-memoryscope-attr}{MemoryScope} ) - 原子操作的内存作用域。$\boxed{13.1}$
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr} ) - 指针的源图块。$\boxed{13.1}$
\item \textbf{mask} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{>}) - 加载操作的掩码。$\boxed{13.1}$
\item \textbf{paddingValue} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e4m3fn}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{fp8e5m2}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{tf32}\texttt{>}) - 加载操作的填充值。$\boxed{13.1}$
\item \textbf{token}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token}）- 用于加载操作的令牌。$\boxed{13.1}$
\item \textbf{optimization\_hints} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-load-ptr-tko-optimizationhints-attr}{OptimizationHints} ) - 操作 $\boxed{13.1}$ 的优化提示


\paragraph{结果}

\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 加载操作的结果。$\boxed{13.1}$
\item \textbf{result\_token} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 加载操作的结果令牌。$\boxed{13.1}$
...
更多信息，请参阅规范中的\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/memory_model.html#section-memory-model}{内存模型章节}。
...
\begin{itemize}
\item \texttt{num\_cta\_in\_cga} - 为 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-entry}{cuda\_tile.entry} 建议 CGA 中的 CTA 数量（必须是小于或等于 16 的 2 的幂）。
\item \texttt{allow\_tma} - 建议是否为\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko}和\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko}使用TMA。
\item \texttt{latency} - 为 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko} 提供的延迟提示。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-load-ptr-tko-0}{cuda\_tile.load\_ptr\_tko\_0}。
...
\begin{itemize}
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 一个全新的令牌，没有任何先前的依赖关系。$\boxed{13.1}$
...
\begin{itemize}
\item \textbf{memory\_ordering\_semantics} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-store-ptr-tko-memoryorderingsemantics-attr}{MemoryOrderingSemantics} ) - 内存排序语义。$\boxed{13.1}$
\item \textbf{memory\_scope} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-store-ptr-tko-memoryscope-attr}{MemoryScope} ) - 可选的内存作用域。$\boxed{13.1}$
\item \textbf{destination} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr} ) - 目标指针图块。$\boxed{13.1}$
\item \textbf{value} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 要存储的值 tile。$\boxed{13.1}$
\item \textbf{mask} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{>}) - 用于选择性存储的可选掩码。$\boxed{13.1}$
\item \textbf{token} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 用于操作排序的可选令牌。$\boxed{13.1}$
\item \textbf{optimization\_hints} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-store-ptr-tko-optimizationhints-attr}{OptimizationHints} ) - 操作 $\boxed{13.1}$ 的优化提示


\paragraph{结果}

\item \textbf{result\_token} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 用于同步的结果令牌。$\boxed{13.1}$
...
\end{itemize}
\textbf{Tile IR} 包含一组类型化的算术运算，这些运算实现了浮点类型上常见的算术操作，关于整数运算请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-integer}{整数运算}。


所有操作均以针对目标架构和设备系列高效的方式实现。在大多数常见情况下，这意味着利用底层硬件的原生浮点运算。由于 \textbf{Tile IR} 的稳定性保证和更高级的编程模型，某些硬件上的某些类型可能被模拟，有关稳定性保证的更多信息以及各设备行为的信息，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/stability.html#section-stability}{稳定性}。
...
基于数据类型支持存在一些限制，这些限制在\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#section-types}{类型系统}章节中有详细说明。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入的浮点类型 tile。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的绝对值。$\boxed{13.1}$
...
逐元素浮点算术运算由目标架构的原生浮点指令执行。若指定了 \texttt{rounding} 修饰符，则特定的舍入模式将应用于结果的每个元素。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-floating-point}{浮点运算}。
...
\item \texttt{source} 和 \texttt{result} 必须具有相同的形状和元素类型（ \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64} > ）。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\begin{itemize}
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{rounding\_mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-addf-roundingmode-attr}{RoundingMode} ) - 该操作的舍入模式。$\boxed{13.1}$
\item \textbf{flush\_to\_zero}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志}）- 如果设置，将次正规输入和结果刷新为保留符号的零。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的总和。$\boxed{13.1}$
...
\item 该操作可以无副作用地推测执行。
\item 该操作是纯的，不执行任何内存副作用。
\item \texttt{lhs}、\texttt{rhs} 和 \texttt{result} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64} >）。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\begin{itemize}
\item \textbf{x} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 x 浮点 tile。 13.2
\item \textbf{y} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入的 y 浮点数 tile。13.2
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 逐元素结果图块。13.2
...
此操作在半精度输入（\texttt{:code:}f16`\texttt{ 和 }:code:\texttt{bf16}\texttt{）上执行时，会在 }:code:\texttt{f32}` 中进行模拟。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-floating-point}{浮点运算}。
...
\item \texttt{x}、\texttt{y} 和 \texttt{result} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64} >）。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-atan2-0}{cuda\_tile.atan2\_0}。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入的浮点型 tile。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的上限值。$\boxed{13.1}$
...
\item 该操作可以无副作用地推测执行。
\item 该操作是纯粹的，不执行任何内存副作用。
\item \texttt{source} 与 \texttt{result} 必须具有相同的形状和元素类型（ \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64} > ）。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-ceil-0}{cuda\_tile.ceil\_0} 获取完整示例列表。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入的浮点瓦片。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的双曲余弦值。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的余弦值。$\boxed{13.1}$
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-cos-0}{cuda\_tile.cos\_0} 查看完整的示例列表。
...
\begin{itemize}
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 作为被除数的输入浮点 tile。$\boxed{13.1}$
\item \textbf{rhs}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64} >）- 作为除数的输入浮点 tile。$\boxed{13.1}$
\item \textbf{rounding\_mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-divf-roundingmode-attr}{RoundingMode} ) - 该操作的舍入模式。$\boxed{13.1}$
\item \textbf{flush\_to\_zero} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志} ) - 若设置，将次正规输入和结果刷新为保留符号的零。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - \texttt{divf} 操作的结果。$\boxed{13.1}$
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入的浮点型 tile。$\boxed{13.1}$
\item \textbf{flush\_to\_zero} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志} ) - 若设置，则将次正规输入和结果刷新为保留符号的零。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 将2提升到输入tile的幂次方所得的结果。$\boxed{13.1}$
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-exp2-0}{cuda\_tile.exp2\_0}。
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的指数。 $\boxed{13.1}$
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-exp-0}{cuda\_tile.exp\_0}。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 向下取整操作的输入瓦片。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 向下取整操作的结果。$\boxed{13.1}$
...
\end{itemize}
请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-floor-0}{cuda\_tile.floor\_0}查看完整示例列表。
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{acc} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 累加器操作数。$\boxed{13.1}$
\item \textbf{rounding\_mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-fma-roundingmode-attr}{RoundingMode} ) - 操作的舍入模式。$\boxed{13.1}$
\item \textbf{flush\_to\_zero} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志} ) - 若设置，将次正规输入和结果刷新为保留符号的零。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的融合乘加运算结果。$\boxed{13.1}$
...
\item 该操作可以无副作用地推测性执行。
\item 该操作是纯的，不执行任何内存副作用。
\item \texttt{lhs}、\texttt{rhs}、\texttt{acc} 和 \texttt{result} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64} >）。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - log2运算的结果。$\boxed{13.1}$
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-log2-0}{cuda\_tile.log2\_0}。
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 对数运算的结果。$\boxed{13.1}$
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{propagate\_nan}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志}）--- 当设置此标志时，如果两个被比较的元素中任意一个是 \texttt{NaN}，则 \texttt{maxf}（或 \texttt{minf}）会返回 \texttt{NaN}。$\boxed{13.1}$
\item \textbf{flush\_to\_zero}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志}）- 如果设置，将次正规输入和结果刷新为保留符号的零。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - \texttt{maxf} 操作的结果。$\boxed{13.1}$
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-maxf-0}{cuda\_tile.maxf\_0} 查看完整示例列表。
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{propagate\_nan}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志}）- 当设置此标志时，如果两个被比较元素中任意一个是 \texttt{NaN}，则 \texttt{maxf}（或 \texttt{minf}）会返回一个 \texttt{NaN}。$\boxed{13.1}$
\item \textbf{flush\_to\_zero} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志} ) - 如果设置，会将次正规输入和结果刷新为保留符号的零。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入图块的最小值。$\boxed{13.1}$
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-minf-0}{cuda\_tile.minf\_0}。
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{rounding\_mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-mulf-roundingmode-attr}{RoundingMode} ) - 该操作的舍入模式。$\boxed{13.1}$
\item \textbf{flush\_to\_zero} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志} ) - 如果设置，将次正规输入和结果刷新为保留符号的零。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的乘积。$\boxed{13.1}$
...
逐元素的浮点算术运算由目标架构的原生浮点指令执行。如果指定了 \texttt{rounding} 修饰符，则特定的舍入模式将应用于结果的每个元素。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-floating-point}{浮点运算}。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入图块。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 取反后的浮点类型 tile。$\boxed{13.1}$
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-negf-0}{cuda\_tile.negf\_0} 获取完整的示例列表。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 基础 tile。$\boxed{13.1}$
\item \textbf{exponent} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 指数 tile。 $\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - pow 操作的结果。$\boxed{13.1}$
...
\item 该操作可以无副作用地推测执行。
\item 该操作是纯粹的，不会产生任何内存副作用。
\item \texttt{result}、\texttt{source} 和 \texttt{exponent} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64} >）。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-pow-0}{cuda\_tile.pow\_0}。
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 除法后的余数。$\boxed{13.1}$
...
\end{itemize}
逐元素的浮点算术运算由目标架构的原生浮点指令执行。如果指定了 \texttt{rounding} 修饰符，特定的舍入模式将应用于结果的每个元素。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-floating-point}{浮点运算}。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 用于计算其倒数平方根的输入瓦片。$\boxed{13.1}$
\item \textbf{flush\_to\_zero} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志} ) - 如果设置，将次正规输入和结果刷新为保留符号的零。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的平方根倒数。$\boxed{13.1}$
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-rsqrt-0}{cuda\_tile.rsqrt\_0} 获取完整的示例列表。
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的双曲正弦值。$\boxed{13.1}$
...
此操作在输入为半精度（\texttt{:code:}f16`\texttt{ 和 }:code:\texttt{bf16}\texttt{）时，会在 }:code:\texttt{f32}` 中进行模拟执行。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-floating-point}{浮点运算}。
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的正弦值。$\boxed{13.1}$
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-sin-0}{cuda\_tile.sin\_0}。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 要计算其平方根的输入 tile。$\boxed{13.1}$
\item \textbf{rounding\_mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-sqrt-roundingmode-attr}{RoundingMode} ) - 该运算的舍入模式。$\boxed{13.1}$
\item \textbf{flush\_to\_zero} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志} ) - 如果设置，将次正规输入和结果刷新为保留符号的零。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入 tile 的平方根。 $\boxed{13.1}$
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{rounding\_mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-subf-roundingmode-attr}{RoundingMode} ) - 该操作的舍入模式。$\boxed{13.1}$
\item \textbf{flush\_to\_zero} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-flag}{标志} ) - 如果设置，将次正规输入和结果刷新为保留符号的零。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 减法运算的结果。$\boxed{13.1}$
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入的浮点类型 tile。$\boxed{13.1}$
\item \textbf{rounding\_mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-tanh-roundingmode-attr}{RoundingMode} ) - 该操作的舍入模式。13.2
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入浮点 tile 的双曲正切值。$\boxed{13.1}$
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-tanh-0}{cuda\_tile.tanh\_0} 查看完整的示例列表。
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{bf16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{f64}\texttt{>}) - 输入浮点 tile 的正切值。$\boxed{13.1}$
...
\end{itemize}
\textbf{Tile IR} 包含一组类型化的算术运算，这些运算实现了对整数瓦片的常见算术操作，有关浮点运算请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-floating-point}{浮点运算}。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入的整数 tile。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入tile的绝对值。$\boxed{13.1}$
...
逐元素整数算术运算由目标架构的原生整数指令执行。默认语义在溢出或下溢时采用环绕语义。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-integer}{整数运算}。
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{overflow} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-addi-integeroverflow-attr}{IntegerOverflow} ) - 操作的溢出行为。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入图块的总和。$\boxed{13.1}$
...
逐元素整数算术运算由目标架构的原生整数指令执行。默认语义为溢出或下溢时的环绕语义。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-integer}{整数运算}。
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{有符号性}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-divi-signedness-attr}{Signedness}）- 将整数解释为 \texttt{有符号} 或 \texttt{无符号} $\boxed{13.1}$
\item \textbf{rounding} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-divi-roundingmode-attr}{RoundingMode} ) - 设置舍入方向（实现向下取整除法/向上取整除法）。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 除法的结果。$\boxed{13.1}$
...
逐元素整数算术运算由目标架构的原生整数指令执行。默认语义是在溢出或下溢时采用环绕语义。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-integer}{整数运算}。
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{有符号性}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-maxi-signedness-attr}{Signedness}）- 将整数解释为 \texttt{有符号} 或 \texttt{无符号} $\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - maxi 操作的结果。$\boxed{13.1}$
...
逐元素整数算术运算由目标架构的原生整数指令执行。默认语义是在溢出或下溢时采用环绕语义。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-integer}{整数}。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-maxi-0}{cuda\_tile.maxi\_0}。
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{有符号性}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-mini-signedness-attr}{Signedness}）- 将整数解释为 \texttt{有符号} 或 \texttt{无符号} $\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入图块的最小值。$\boxed{13.1}$
...
逐元素整数算术运算由目标架构的原生整数指令执行。默认语义是溢出或下溢时的环绕语义。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-integer}{整数}。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-mini-0}{cuda\_tile.mini\_0}。
...
\begin{itemize}
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧输入的整数 tile。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧输入的整数图块。$\boxed{13.1}$
\item \textbf{overflow} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-muli-integeroverflow-attr}{IntegerOverflow} ) - 操作的溢出行为。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入 tile 的乘积。$\boxed{13.1}$
...
逐元素整数算术运算由目标架构的原生整数指令执行。默认语义是在上溢或下溢时采用环绕语义。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-integer}{整数运算}。
...
\begin{itemize}
\item \textbf{x} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧输入的整数 tile。$\boxed{13.1}$
\item \textbf{y} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧输入的整数 tile。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入tile乘积的最高有效位。$\boxed{13.1}$
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-mulhii-0}{cuda\_tile.mulhii\_0}。
...
\begin{itemize}
\item \textbf{source} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入的整数瓦片。$\boxed{13.1}$
\item \textbf{overflow} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-negi-integeroverflow-attr}{IntegerOverflow} ) - 操作的溢出行为。13.2
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 取反后的整数 tile。$\boxed{13.1}$
...
逐元素整数算术运算由目标架构的原生整数指令执行。默认语义为溢出或下溢时的环绕语义。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-integer}{整数}。
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-negi-0}{cuda\_tile.negi\_0} 查看完整示例列表。
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入 tile 的按位或运算结果。$\boxed{13.1}$
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{有符号性}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-remi-signedness-attr}{Signedness}）- 将整数解释为 \texttt{有符号} 或 \texttt{无符号} $\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 除法后的余数。$\boxed{13.1}$
...
\item 该操作可以无副作用地推测执行。
\item 该操作是纯粹的，不会产生任何内存副作用。
\item \texttt{result}、\texttt{lhs} 和 \texttt{rhs} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64} >）。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\begin{itemize}
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧操作数（移位量）。$\boxed{13.1}$
\item \textbf{overflow} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-shli-integeroverflow-attr}{IntegerOverflow} ) - 操作的溢出行为。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左移操作的结果。$\boxed{13.1}$
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧操作数（移位量）。$\boxed{13.1}$
\item \textbf{有符号性}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-shri-signedness-attr}{有符号性}）- 将整数解释为 \texttt{有符号} 或 \texttt{无符号} $\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右移操作的结果。$\boxed{13.1}$
...
\item \textbf{lhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
\item \textbf{overflow} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-subi-integeroverflow-attr}{IntegerOverflow} ) - 操作的溢出行为。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 减法运算的结果。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入 tile 的按位异或结果。$\boxed{13.1}$
...
逐元素整数算术运算由目标架构的原生整数指令执行。默认语义是溢出或下溢时的环绕语义。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-integer}{整数运算}。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-xori-0}{cuda\_tile.xori\_0}。
...
\begin{itemize}
\item \textbf{lhs}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64} >）- 左侧操作数。$\boxed{13.1}$
\item \textbf{rhs} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 右侧操作数。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i8}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i16}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i32}\texttt{|}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i64}\texttt{>}) - 输入 tile 的按位与运算结果。$\boxed{13.1}$
...
逐元素整数算术运算由目标架构的原生整数指令执行。默认语义在溢出或下溢时采用环绕语义。更多详情请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-group-integer}{整数}。
...
\begin{itemize}
\item \textbf{memory\_ordering\_semantics} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-atomic-cas-tko-memoryorderingsemantics-attr}{MemoryOrderingSemantics} ) - 原子操作的内存排序语义。$\boxed{13.1}$
\item \textbf{memory\_scope} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-atomic-cas-tko-memoryscope-attr}{MemoryScope} ) - 原子操作的内存作用域。$\boxed{13.1}$
\item \textbf{指针}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr}）- 指向要执行原子比较与交换操作的内存位置的指针。$\boxed{13.1}$
\item \textbf{cmp} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 要进行比较的值。$\boxed{13.1}$
\item \textbf{val} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 要交换进去的值。$\boxed{13.1}$
\item \textbf{mask} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{>}) - 原子操作的掩码。$\boxed{13.1}$
\item \textbf{token} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 原子操作的令牌。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 原子操作的结果。$\boxed{13.1}$
\item \textbf{result\_token} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 原子操作的结果令牌。$\boxed{13.1}$
...
\item \texttt{cmp}、\texttt{val} 和 \texttt{result} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}）。
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-atomic-cas-tko-0}{cuda\_tile.atomic\_cas\_tko\_0} 获取完整示例列表。
...
\begin{itemize}
\item \textbf{memory\_ordering\_semantics} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-atomic-rmw-tko-memoryorderingsemantics-attr}{MemoryOrderingSemantics} ) - 加载操作的内存排序语义。$\boxed{13.1}$
\item \textbf{memory\_scope} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-atomic-rmw-tko-memoryscope-attr}{MemoryScope} ) - 原子操作的内存作用域。$\boxed{13.1}$
\item \textbf{指针}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr}）- 执行原子操作的指针瓦片。$\boxed{13.1}$
\item \textbf{mode} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-atomic-rmw-tko-atomicrmwmode-attr}{AtomicRMWMode} ) - 原子操作模式（例如：add、max、min 等）。$\boxed{13.1}$
\item \textbf{arg} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 用于原子操作的值块。$\boxed{13.1}$
\item \textbf{mask} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#subsection-element-types}{i1}\texttt{>}) - 加载操作的掩码。$\boxed{13.1}$
\item \textbf{token} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 原子操作的令牌。$\boxed{13.1}$
...
\begin{itemize}
\item \texttt{arg} 和 \texttt{result} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}）。
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-atomic-rmw-tko-0}{cuda\_tile.atomic\_rmw\_tko\_0} 获取完整示例列表。
...
视图是一种与内存中的张量进行交互的结构化方式。相关内容在类型章节 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#sub-sec-view-types}{张量视图} 和语义章节 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/semantics.html#section-semantics-sub-section-views}{视图} 中均有描述。



视图是\textbf{Tile IR}中与全局内存交互的主要方式。常见的模式是使用\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-make-tensor-view}{cuda\_tile.make\_tensor\_view}从指针构建一个\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{张量视图}，然后使用\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko}和\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko}操作对其进行读写。对于较大的张量，加载整个张量效率不高，因此我们提供了子视图\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{分区视图}，允许用户对\texttt{tensor\_view}进行分块处理。
...
\begin{itemize}
\item \textbf{src} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-view-type}{视图类型} ) - 源视图类型。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{Variadic}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{any} > > ) - 索引空间的形状，每个值表示对应维度的大小。$\boxed{13.1}$
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-get-index-space-shape-0}{cuda\_tile.get\_index\_space\_shape\_0}。
...
\begin{itemize}
\item \textbf{src} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{tensor\_view} ) - 源张量视图。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{Variadic}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{any} > > ) - 张量的形状，每个值代表对应维度的大小。$\boxed{13.1}$
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-get-tensor-shape-0}{cuda\_tile.get\_tensor\_shape\_0}。
...
\begin{itemize}
\item \textbf{memory\_ordering\_semantics}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-load-view-tko-memoryorderingsemantics-attr}{MemoryOrderingSemantics}）- 加载操作的内存排序语义。$\boxed{13.1}$
\item \textbf{memory\_scope} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-load-view-tko-memoryscope-attr}{MemoryScope} ) - 原子操作的内存作用域。$\boxed{13.1}$
\item \textbf{view} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-view-type}{view\_type} ) - 加载图块所用的视图。$\boxed{13.1}$
\item \textbf{index} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{Variadic}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{any} > > ) - 要从视图加载的所需元素的 n 维索引。$\boxed{13.1}$
\item \textbf{token}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token}）- 用于加载操作的可选令牌。$\boxed{13.1}$
\item \textbf{optimization\_hints} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-load-view-tko-optimizationhints-attr}{OptimizationHints} ) - 操作 $\boxed{13.1}$ 的优化提示
...
\item \textbf{tile} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 加载的图块。$\boxed{13.1}$
\item \textbf{result\_token} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 结果令牌。$\boxed{13.1}$
...
例如，\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{分区视图}将一个\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{张量视图}划分为一个由相同大小图块组成的网格。该视图索引网格中一个被分区的图块。



对于给定的视图，索引的秩必须与视图索引空间的秩相匹配。有效索引的空间取决于传递给操作的视图。

例如，\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{分区视图}的索引空间等于分区图块的秩。
...
越界访问根据\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{分区视图}的语义处理。
...
更多信息，请参阅规范中的\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/memory_model.html#section-memory-model}{内存模型部分}。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-load-view-tko-0}{cuda\_tile.load\_view\_tko\_0}。
...
\begin{itemize}
\item \textbf{tensor\_view} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{tensor\_view} ) - 用于创建分区视图的源张量视图。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{partition\_view} ) - 创建的划分视图。$\boxed{13.1}$
...
\end{itemize}
\texttt{make\_partition\_view} 操作从 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{tensor\_view} 创建一个 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{partition\_view}。有关分区视图的更多详细信息，请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{Partition View}。
...
生成的分区视图可以使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko} 加载，并使用 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko} 存储。



视图内存选项作用于分区视图的计算索引空间，详见\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{Tensor View}和\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{Partition View}获取详细语义。
...
\end{itemize}
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-make-partition-view-0}{cuda\_tile.make\_partition\_view\_0} 查看完整的示例列表。
...
\begin{itemize}
\item \textbf{base} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-ptr}{ptr}\texttt{>}) - 指向全局内存某一部分的标量基指针。$\boxed{13.1}$
\item \textbf{dynamicShape}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{图块}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{任意类型} > >）- 表示视图形状的数值数组，可以是完全动态的。$\boxed{13.1}$
\item \textbf{dynamicStrides}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{瓦片}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{任意类型} > >）- 表示视图步长的值数组，可以是完全动态的。$\boxed{13.1}$
...
\item \textbf{result}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{tensor\_view}）- 构造的 tensor\_view。$\boxed{13.1}$
...
\end{itemize}
\texttt{make\_tensor\_view} 操作从一个全局内存指针、动态形状和动态步幅构建一个 \texttt{tensor\_view}。更多详情请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{Tensor View}。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-make-tensor-view-0}{cuda\_tile.make\_tensor\_view\_0}。
...
\begin{itemize}
\item \textbf{memory\_ordering\_semantics} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-store-view-tko-memoryorderingsemantics-attr}{MemoryOrderingSemantics} ) - 存储操作的内存排序语义。$\boxed{13.1}$
\item \textbf{memory\_scope} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-store-view-tko-memoryscope-attr}{MemoryScope} ) - 存储操作的内存作用域。$\boxed{13.1}$
\item \textbf{tile} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile} ) - 要存储的瓦片。$\boxed{13.1}$
\item \textbf{视图} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-view-type}{视图类型} ) - 用于存储图块的视图。$\boxed{13.1}$
\item \textbf{index} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{瓦片}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{任意} > > ) - 视图中目标瓦片的索引。$\boxed{13.1}$
\item \textbf{token} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 用于操作顺序的可选令牌。$\boxed{13.1}$
\item \textbf{optimization\_hints} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-store-view-tko-optimizationhints-attr}{OptimizationHints} ) - 操作 $\boxed{13.1}$ 的优化提示
...
例如，\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{分区视图}将\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tensor-view}{张量视图}划分为一个由等尺寸图块组成的网格。该视图用于索引网格中一个被分区的图块。
...
越界访问根据\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-partition-view}{分区视图}的语义进行处理。
...
\item \texttt{num\_cta\_in\_cga} - 为 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-entry}{cuda\_tile.entry} 建议 CGA 中的 CTA 数量（必须是小于或等于 16 的 2 的幂次）。
\item \texttt{allow\_tma} - 建议是否对 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko} 使用 TMA。
\item \texttt{latency} - 用于 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-load-view-tko}{cuda\_tile.load\_view\_tko} 和 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-cuda-tile-store-view-tko}{cuda\_tile.store\_view\_tko} 的延迟提示。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-store-view-tko-0}{cuda\_tile.store\_view\_tko\_0}。
...
\begin{itemize}
\item \textbf{value} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{Any} ) - 要附加谓词的值。$\boxed{13.1}$
\item \textbf{predicate} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#op-attribute-cuda-tile-assume-assumepredicate-attr}{AssumePredicate} ) - 要附加到该值的谓词。$\boxed{13.1}$
...
\item \textbf{result} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{Any} ) - 带有附加谓词的值。$\boxed{13.1}$
...
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-assume-example-bounded-0}{cuda\_tile.assume\_example\_Bounded\_0}。
...
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-assume-example-divby-0}{cuda\_tile.assume\_example\_DivBy\_0}。
...
请参阅 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-assume-example-divby-1}{cuda\_tile.assume\_example\_DivBy\_1} 获取完整示例列表。
...
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-assume-example-divby-2}{cuda\_tile.assume\_example\_DivBy\_2}。
...
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-assume-example-divby-3}{cuda\_tile.assume\_example\_DivBy\_3}。
...
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-assume-example-divby-4}{cuda\_tile.assume\_example\_DivBy\_4}。
...
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-assume-example-sameelements-0}{cuda\_tile.assume\_example\_SameElements\_0}。
...
\begin{itemize}
\item \texttt{value} 和 \texttt{result} 必须具有相同的形状和元素类型（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-any}{Any}）。
\item 操作的结果类型可以从其操作数和属性中推断出来。
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-assume-0}{cuda\_tile.assume\_0}。
...
\begin{itemize}
\item \textbf{str} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-string}{字符串} ) - 格式字符串。$\boxed{13.1}$
\item \textbf{args} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-variadic}{可变参数}\texttt{<}\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/types.html#type-tile}{tile}\texttt{>}) - 要格式化并打印的参数。$\boxed{13.1}$
\item \textbf{token}（\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token}）- 用于操作顺序的可选令牌。13.2
...
\item \textbf{result\_token} ( \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/operations.html#type-token}{token} ) - 用于同步的结果令牌。13.2
...
\end{itemize}
完整示例列表请参见 \href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/appendix.html#example-cuda-tile-print-tko-0}{cuda\_tile.print\_tko\_0}。
...
调试信息对于理解和诊断 \textbf{Tile IR} 程序的行为至关重要。调试信息是一种元数据，它将生成的代码映射回原始源代码，为 \textbf{Tile IR} 用户提供更具信息性和直观性的调试体验，并允许用户快速识别和修复 \textbf{Tile IR} 程序中的问题。本节介绍在使用 MLIR 方言时，如何在 \textbf{Tile IR} 程序中包含和使用调试信息。有关如何在字节码中直接生成此信息的说明，请参阅\href{https://docs.nvidia.com/cuda/tile-ir/latest/sections/bytecode.html#section-bytecode}{字节码部分}。
