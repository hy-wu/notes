...
/**
 * BAMPS GPU Implementation - Phase 2 (Complete)
 * Dynamic particle management, Local Debye Mass, 2<->3 processes.
 */
...
__global__ void compute_cell_ids_kernel(float4* pos, int* is_alive, int* cell_ids, int num_slots, float dx, int IX, float box_size) {
    ...
    if (idx < num_slots) {
        if (is_alive[idx]) {
            cell_ids[idx] = get_cell_id(pos[idx], dx, IX, box_size);
        } else {
            cell_ids[idx] = 2000000000; 
        }
    }
}
...
// 4. 2->2 and 2->3 Collision Kernel
__global__ void collide_pairs_kernel(
    ...
    int max_slots, int num_cells, float dt, float dv, int testpartcl, float as) 
{
    ...
    for (int i = start; i < end; i++) {
        for (int j = i + 1; j < end; j++) {
            ...
            float sigma23 = 0.5f * sigma22; // Inelastic part
            float total_prob = v_rel * (sigma22 + sigma23) * dt / (dv * testpartcl);

            if (curand_uniform(&local_state) < total_prob) {
                if (curand_uniform(&local_state) < (sigma22 / (sigma22 + sigma23))) {
                    ...
                    if (new_idx < max_slots) {
                        sorted_pos[new_idx] = sorted_pos[i];
                        sorted_mom[new_idx] = boost(P3_cm, minus_beta);
                        sorted_is_alive[new_idx] = 1;
                    }
                }
            }
        }
...
// 5. 3->2 Collision Kernel (Random Subset Sampling)
__global__ void collide_triplets_kernel(
    float4* sorted_pos, float4* sorted_mom, int* sorted_is_alive,
    int* cell_starts, int* cell_ends, float* cell_md2,
    ...
    int max_slots, int num_cells, float dt, float dv, int testpartcl, float as) 
{
    ...
    curandState local_state = rand_states[cell_idx + num_cells]; // Use separate states
    
    // Evaluate M random triplets
    int M = 10; 
    float n_triplets = (float)n * (n - 1) * (n - 2) / 6.0f;
    float amplification = n_triplets / (float)M;
    ...
    for (int k = 0; k < M; k++) {
        ...
        // I32 = 192 * pi^2 * sigma23 (from paper)
        // Simplified P32 calculation
        float prob32 = amplification * (1.0f / (8.0f * P1.w * P2.w * P3.w)) * (10.0f) * dt / (dv * dv * testpartcl * testpartcl);
        ...
        if (curand_uniform(&local_state) < prob32) {
            ...
            float3 minus_beta = make_float3(-beta.x, -beta.y, -beta.z);
            sorted_mom[i] = boost(P1_cm, minus_beta); sorted_mom[j] = boost(P2_cm, minus_beta);
            sorted_is_alive[l] = 0; // Annihilate 3rd particle
        }
    }
...
struct InvP {
    __device__ float operator()(const float4& m) const {
        float p = sqrtf(m.x * m.x + m.y * m.y + m.z * m.z);
        return (p > 1e-6f) ? (1.0f / p) : 0.0f;
    }
};
...
public:
    ...
    BAMPS_GPU(int initial_n, float box_sz, float cell_sz, int tp) 
        ...
        max_slots = initial_n * 4; IX = (int)(box_size / dx); num_cells = IX * IX * IX; dv = dx * dx * dx;
    ...
    void compute_local_md2() {
        ...
        update_cell_md2_kernel<<<(num_found + 255) / 256, 256>>>(
            thrust::raw_pointer_cast(keys.data()),
            thrust::raw_pointer_cast(sums.data()),
            thrust::raw_pointer_cast(cell_md2.data()),
            num_found, factor, num_cells
        );
    }
    ...
    void evolve(float dt) {
        ...
        collide_pairs_kernel<<<blocks_c, threadsPerBlock>>>(thrust::raw_pointer_cast(pos.data()), thrust::raw_pointer_cast(mom.data()), thrust::raw_pointer_cast(is_alive.data()), thrust::raw_pointer_cast(cell_starts.data()), thrust::raw_pointer_cast(cell_ends.data()), thrust::raw_pointer_cast(cell_md2.data()), thrust::raw_pointer_cast(rand_states.data()), d_atomic_counter, max_slots, num_cells, dt, dv, testpartcl, alpha_s);
        collide_triplets_kernel<<<blocks_c, threadsPerBlock>>>(thrust::raw_pointer_cast(pos.data()), thrust::raw_pointer_cast(mom.data()), thrust::raw_pointer_cast(is_alive.data()), thrust::raw_pointer_cast(cell_starts.data()), thrust::raw_pointer_cast(cell_ends.data()), thrust::raw_pointer_cast(cell_md2.data()), thrust::raw_pointer_cast(rand_states.data()), max_slots, num_cells, dt, dv, testpartcl, alpha_s);
    }
};

int main() {
    int initial_n = 10000; float box_size = 5.0f; float cell_size = 1.0f; int testpartcl = 100; float dt = 0.01f;
    BAMPS_GPU sim(initial_n, box_size, cell_size, testpartcl);
    thrust::host_vector<float4> h_mom(sim.max_slots, make_float4(0,0,0,0)); thrust::host_vector<float4> h_pos(sim.max_slots, make_float4(0,0,0,0)); thrust::host_vector<int> h_alive(sim.max_slots, 0);
    for(int i=0; i<initial_n; ++i) {
        float p = 2.0f; h_mom[i] = make_float4(0, 0, p, p);
        h_pos[i] = make_float4((float)rand()/RAND_MAX * box_size - box_size/2.0f, (float)rand()/RAND_MAX * box_size - box_size/2.0f, (float)rand()/RAND_MAX * box_size - box_size/2.0f, 0.0f);
        h_alive[i] = 1;
    }
    sim.mom = h_mom; sim.pos = h_pos; sim.is_alive = h_alive;
    printf("Starting Phase 2 Simulation (Creation & Annihilation enabled)...\n");
    for (int i = 0; i < 50; i++) {
        sim.evolve(dt);
        if (i % 10 == 0) {
            int current_n = thrust::count(sim.is_alive.begin(), sim.is_alive.end(), 1);
            printf("Step %d: Active Particles = %d\n", i, current_n);
        }
    }
    return 0;
}
