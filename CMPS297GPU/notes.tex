...
\hline
\textbf{GPU Name} & \textbf{K40} & \textbf{M40} & \textbf{P100} & \textbf{V100} & \textbf{A100} & \textbf{3090} & \textbf{4090} & \textbf{H100} & \textbf{5090} \\ \hline
Arch. & Kepler & Maxw. & Pasc. & Volta & Amp. & Amp. & Ada & Hopp. & Black. \\ \hline
CC & 3.5 & 5.2 & 6.0 & 7.0 & 8.0 & 8.6 & 8.9 & 9.0 & 10.0 \\ \hline
SMs & 15 & 24 & 56 & 80 & 108 & 82 & 128 & 132 & 170 \\ \hline
Cores/SM & 192 & 128 & 64 & 64 & 64 & 128 & 128 & 128 & 128 \\ \hline
Threads/SM & 2048 & 2048 & 2048 & 2048 & 2048 & 1536 & 1536 & 2048 & 2048 \\ \hline
Blocks/SM & 16 & 32 & 32 & 32 & 32 & 16 & 32 & 32 & 32 \\ \hline
Threads/Blk & 1024 & 1024 & 1024 & 1024 & 1024 & 1024 & 1024 & 1024 & 1024 \\ \hline
Regs/SM & 64K & 64K & 64K & 64K & 64K & 64K & 64K & 64K & 64K \\ \hline
SMem/SM & 48KB & 96KB & 64KB & 96KB & 164KB & 100KB & 100KB & 228KB & 228KB \\ \hline
\caption{There may be mistakes in the table}
\end{longtable}
% }
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{images/image-mem.pdf}
...
assign multiple units of parallelism per thread
\begin{itemize}
...
\end{itemize}
\end{itemize}
\subsection{Checklist of Common Optimizations}

{
% \small
\setlength{\tabcolsep}{3pt}
\setlength{\extrarowheight}{2pt}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.29\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}}
\hline
\textbf{Optimization} & \textbf{Improves} & \textbf{Impact on Cores} & \textbf{Impact on Memory} \\ \hline
...
\subsection{Tensions Between Optimizations}
...
\begin{itemize}
\item a straightforward implementation: a thread per output element
\end{itemize}
observation: the mask is small, constant, and accessed by all threads,\\
$\implies$ store in constant memory
\begin{itemize}
...
\begin{itemize}
\item tree reduction on each segment $\to$ partial sums
\begin{itemize}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{images/reduction_coarsening.pdf}
\caption{thread coarsening for reduction}
\end{figure}
\begin{itemize}
...
%\newpage
\section{Scan (Kogge-Stone, Brent-Kung)}
scan operation
takes
\begin{itemize}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{images/image.pdf}
\caption{segmented scan (also called hierarchical scan)}
\end{figure}
...
%\newpage
\section{Merge}
parallelization of ordered merge
\begin{itemize}
...
\end{itemize}
\item extract bit $\to$ exclusive scan $\to$ destination index $\to$ take and store

\end{itemize}
\end{itemize}
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{images/image-4.pdf}
...
%\newpage
\section{Sparse Matrix (COO, CSR, ELL, JDS)}
...
\begin{figure}
    ...
    \centering
    \includegraphics[width=\linewidth]{images/image-11.png}
    \caption{SpMV/COO}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
    \centering
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{images/image-12.png}
\caption{SpMV ELL}
\end{figure}
ELL tradeoffs
\begin{itemize}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{images/image-16.pdf}
\caption{Vertex-Centric BFS (Top-Down)}
\end{figure}
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{images/image-15.pdf}
...
\end{itemize}
\item optimization: if consecutive levels have few enough vertices that can be executed by a thread block:
\begin{itemize}
\item execute multiple levels in one single-block grid
\item and synchronize between levels using \texttt{\_\_syncthreads()}
...
\item \texttt{cudaError\_t cudaMemcpyAsync(void *dst, const void *src, size\_t count,}\\
\hspace*{14.5em} \texttt{cudaMemcpyKind kind, cudaStream\_t stream=0)}
...
\vspace{1em}\hspace{1em} Nested Parallelism: Dynamic parallelism is useful for programming applications with nested parallelism, where each thread discovers more work that can be parallelized; particularly useful when the amount of nested work is unknown, so enough threads cannot be launched up front.

\vspace{1em} Applications of Dynamic Parallism
\begin{itemize}
...
\centering
\includegraphics[width=\linewidth]{images/image-22.png}
...
\item The limit can be increased by setting\\
\texttt{cudaDeviceSetLimit(cudaLimitDevRuntimePendingLaunchCount, <new limit>)}
...
\begin{itemize}
\item other GPUs: AMD (Radeon), ARM (Mali), Intel, ...
\item integrated GPUs
\item FPGA
\item ...
\end{itemize}
