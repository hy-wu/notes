...
template <typename Kin, typename Bound, typename Pot, typename Therm>
class Simulation {
public:
    int n, num_cells, IX;
    ...
    void step() {
        if constexpr (std::is_same_v<Therm, GlobalScalingThermostat>) {
            ...
            reduce_ke_kernel<<<(n + 255) / 256, 256>>>(d_particles, n, d_ke_sum);
...
int main(int argc, char** argv) {
    ...
    Simulation<ClassicalKinematics, ReflectiveWall, LennardJones, GlobalScalingThermostat> sim(N, box, sig, eps, T_target);
